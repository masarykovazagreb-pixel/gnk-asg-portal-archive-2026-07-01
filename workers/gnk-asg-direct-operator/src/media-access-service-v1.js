...
export const MEDIA_ACCESS_VERSION='GNK_ASG_MEDIA_ACCESS_SERVICE_V3_20260628';
...
async function adminState(request,env){return adminAccess(request,env)}
...
function requestMeta(request){return{ip:clean(request.headers.get('cf-connecting-ip')),userAgent:clean(request.headers.get('user-agent')).slice(0,300),requestId:clean(request.headers.get('cf-ray')||request.headers.get('x-request-id')).slice(0,120)}}
...
async function requireAdmin(request,env){
  const state=await adminState(request,env);
  return state.ok?state:null;
}
async function adminList(request,env){
  const admin=await requireAdmin(request,env);if(!admin)return json({ok:false,error:'unauthorized'},401);
  ...
  return json({ok:true,version:MEDIA_ACCESS_VERSION,authMode:admin.mode,count:rows.length,items:rows});
}
async function auditList(request,env){
  ...
  return json({ok:true,authMode:admin.mode,count:rows.length,items:rows});
}
async function issueCode(request,env){
  ...
  if(live&&body.confirm!=='ISSUE_APPROVED_MEDIA_ACCESS')return json({ok:false,error:'confirmation_required',required:'ISSUE_APPROVED_MEDIA_ACCESS'},409);
  const code=randomDigits(),stamp=now(),expiresAt=new Date(Date.now()+CODE_TTL_MINUTES*60000).toISOString();
  await db.prepare(`UPDATE media_access_codes SET status='REPLACED',revoked_at=?,updated_at=? WHERE mail_code=? AND status IN ('ACTIVE','DRY_RUN_READY')`).bind(stamp,stamp,mailCode).run();
  ...
  await accessEvent(env,live?'access_code_issued':'access_code_dry_run',{mailCode,email,detail:{expiresAt,live,adminMode:admin.mode,...requestMeta(request)}});
...
async function revoke(request,env){
  ...
  await accessEvent(env,'access_revoked',{mailCode,detail:{reason:clean(body.reason)||'admin',codes:codes.meta?.changes||0,sessions:sessions.meta?.changes||0,adminMode:admin.mode,...requestMeta(request)}});
...
async function verifyCode(request,env){
  ...
  const stamp=now();
  if(Date.parse(row.expires_at)<=Date.now()){await db.prepare(`UPDATE media_access_codes SET status='EXPIRED',updated_at=? WHERE id=? AND status='ACTIVE'`).bind(stamp,row.id).run();await accessEvent(env,'access_code_expired',{mailCode,email,detail:requestMeta(request)});return json({ok:false,error:'code_expired'},410)}
  if(Number(row.attempts||0)>=MAX_ATTEMPTS){await db.prepare(`UPDATE media_access_codes SET status='LOCKED',updated_at=? WHERE id=? AND status='ACTIVE'`).bind(stamp,row.id).run();return json({ok:false,error:'code_locked'},429)}
  if(await hashCode(env,mailCode,email,code)!==row.code_hash){
    const attempts=Number(row.attempts||0)+1,locked=attempts>=MAX_ATTEMPTS;
    await db.prepare(`UPDATE media_access_codes SET attempts=?,status=?,updated_at=? WHERE id=? AND status='ACTIVE'`).bind(attempts,locked?'LOCKED':'ACTIVE',stamp,row.id).run();
    await accessEvent(env,locked?'access_code_locked':'access_code_rejected',{mailCode,email,detail:{attempts,remaining:Math.max(0,MAX_ATTEMPTS-attempts),...requestMeta(request)}});
    return json({ok:false,error:locked?'code_locked':'invalid_code',attemptsRemaining:Math.max(0,MAX_ATTEMPTS-attempts)},locked?429:401);
  }
  const consumed=await db.prepare(`UPDATE media_access_codes SET status='USED',used_at=?,updated_at=? WHERE id=? AND status='ACTIVE' AND attempts<? AND expires_at>?`).bind(stamp,stamp,row.id,MAX_ATTEMPTS,stamp).run();
  if(Number(consumed.meta?.changes||0)!==1)return json({ok:false,error:'code_already_used_or_changed'},409);
  const token=randomToken(),expiresAt=new Date(Date.now()+SESSION_TTL_HOURS*3600000).toISOString();
  ...
  await accessEvent(env,'access_login',{mailCode,email,detail:{expiresAt,...requestMeta(request)}});
...
async function logout(request,env){
  ...
  if(token){
    const db=await ensureAccessSchema(env),digest=await hashSession(env,token);
    const session=await db.prepare(`SELECT mail_code,email FROM media_access_sessions WHERE session_hash=? LIMIT 1`).bind(digest).first();
    await db.prepare(`UPDATE media_access_sessions SET revoked_at=? WHERE session_hash=? AND revoked_at IS NULL`).bind(now(),digest).run();
    if(session)await accessEvent(env,'access_logout',{mailCode:session.mail_code,email:session.email,detail:requestMeta(request)});
  }
