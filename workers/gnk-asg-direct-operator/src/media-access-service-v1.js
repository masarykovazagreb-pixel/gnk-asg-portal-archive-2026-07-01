...
async function ensureAccessSchema(env){
  ...
  await db.batch([
    db.prepare(`CREATE TABLE IF NOT EXISTS media_access_codes (id TEXT PRIMARY KEY,mail_code TEXT NOT NULL,email TEXT NOT NULL,code_hash TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'ACTIVE',expires_at TEXT NOT NULL,used_at TEXT,attempts INTEGER NOT NULL DEFAULT 0,sent_at TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_media_access_codes_lookup ON media_access_codes(mail_code,email,status,expires_at)`),
    db.prepare(`CREATE TABLE IF NOT EXISTS media_access_sessions (id TEXT PRIMARY KEY,mail_code TEXT NOT NULL,email TEXT NOT NULL,session_hash TEXT NOT NULL UNIQUE,expires_at TEXT NOT NULL,created_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,ended_at TEXT)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_media_access_sessions_lookup ON media_access_sessions(session_hash,expires_at)`),
    db.prepare(`CREATE TABLE IF NOT EXISTS media_access_events (id TEXT PRIMARY KEY,event_type TEXT NOT NULL,mail_code TEXT,email TEXT,detail_json TEXT,created_at TEXT NOT NULL)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_media_access_events_mail_code ON media_access_events(mail_code,created_at)`)
  ]);
  return db;
}
async function accessEvent(env,eventType,data={}){
  const db=await ensureAccessSchema(env);
  await db.prepare(`INSERT INTO media_access_events(id,event_type,mail_code,email,detail_json,created_at) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),eventType,clean(data.mailCode),clean(data.email).toLowerCase(),JSON.stringify(data.detail||{}),now()).run();
}
...
async function issueCode(request,env){
  ...
  await db.prepare(`UPDATE media_access_codes SET status='REPLACED',updated_at=? WHERE mail_code=? AND status='ACTIVE'`).bind(stamp,mailCode).run();
...
async function sessionInfo(request,env){
  ...
  const session=await db.prepare(`SELECT * FROM media_access_sessions WHERE session_hash=? AND ended_at IS NULL ORDER BY created_at DESC LIMIT 1`).bind(digest).first();
...
async function logout(request,env){
  ...
  if(token){const db=await ensureAccessSchema(env);await db.prepare(`UPDATE media_access_sessions SET ended_at=? WHERE session_hash=?`).bind(now(),await hashSession(env,token)).run();}
