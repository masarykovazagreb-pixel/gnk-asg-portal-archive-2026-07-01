...
export const VERSION='GNK_ASG_PUBLIC_V13_EDITORIAL_CONTACT_MEDIA_20260627';
...
async function patch(response,path,request){
  ...
  const market=MARKET_PATHS.has(path),editorial=isEditorial(path),contact=path==='/contact',media=path==='/media-kit';
  if(!market&&!editorial&&!contact&&!media)return response;
