...
export default{
  async fetch(request,env,ctx){const path=pathOf(request);if(request.method==='GET'&&INDEX_PATHS.has(path))return serveIndex(path,request,env);return patch(await app.fetch(request,env,ctx),path,request)},
