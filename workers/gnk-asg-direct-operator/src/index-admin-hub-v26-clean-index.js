...
async function patch(response,path,request){
  ...
  if(docs){if(!html.includes('public-docs-v20.css'))html=html.replace('</head>',DOC_STYLE+'</head>');const kind=path.includes('downloads')?'downloads':'legal';html=html.replace(/<body([^>]*)>/i,(match,attrs)=>`<body${attrs} class="${String(attrs).match(/class=["']([^"']*)["']/i)?.[1]||''} gnk-docs-v20 gnk-docs-${kind}">`.replace(/class="\s+/,'class="').replace(/\s+"/,'"'));headers.set('x-gnk-asg-docs-layout',`UNIFIED_${kind.toUpperCase()}_V20`)}
