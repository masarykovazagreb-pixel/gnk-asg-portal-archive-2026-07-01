...
export const VERSION='GNK_ASG_MEDIA_COMMAND_CENTER_V2_20260626_R2';
...
function assess(contact,control={},suppressed=false){
  const email=clean(control.to_email||contact.email||contact.toEmail||contact.to_email);
  ...
  const toEmail=clean(control.to_email)||addresses[0]||email;
  const ccEmail=clean(control.cc_email)||addresses.slice(1).join(', ');
  let operationalStatus='READY';let blockedReason='';
...
function validateDataset(input){
  ...
  const normalized=items.map(normalizeImported);const errors=[];const codes=new Set();const outlets=new Set();
  normalized.forEach((item,index)=>{if(!item.mailCode)errors.push({index,error:'mailCode_missing'});if(!item.outlet)errors.push({index,error:'outlet_missing'});if(item.mailCode&&codes.has(item.mailCode))errors.push({index,error:'duplicate_mailCode',value:item.mailCode});codes.add(item.mailCode);outlets.add(lower(item.outlet));});
...
async function importApply(request,env){
  ...
  const checked=validateDataset(body);if(!checked.ok)return json({ok:false,...checked},400);
  const db=await ensureSchema(env),stamp=now(),existing=(await db.prepare(`SELECT mail_code FROM media_outreach_contacts`).all()).results||[],existingCodes=new Set(existing.map(row=>row.mail_code));
  let created=0,updated=0;
  for(const item of checked.items){
    const exists=existingCodes.has(item.mailCode);exists?updated++:created++;
  ...
  await db.prepare(`INSERT INTO media_contact_imports(id,source_version,contact_count,created_count,updated_count,unchanged_count,imported_by,detail_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)`)
    .bind(crypto.randomUUID(),'GNK_ASG_MEDIA_HANDOFF_2026-06-26',checked.summary.count,created,updated,0,clean(body.importedBy)||'ADMIN',JSON.stringify(checked.summary),stamp).run();
  return json({ok:true,imported:{...checked.summary,created,updated},readiness:await readinessSummary(env)});
}
...
async function intercept(request,env,ctx){
  ...
  if(path===`${API}/status`&&request.method==='GET'){try{const payload=await response.clone().json();return json({...payload,mediaCommandV1:V1_VERSION,mediaCommandV2:VERSION,readiness:await readinessSummary(env)});}catch{return response;}}
