...
const VERSION='GNK_ASG_ADMIN_HUB_V22_NEWS_V19_STATIC_CODE_SHOWCASE_20260627';
...
function showcaseMarkup(images,english){
  ...
  const visualLabel=english?'Rotating campaign visuals':'Izmjena kampanjskih vizuala';
...
async function patchIndexShowcase(response,path,request,env){
  if(!['/','/en'].includes(path)||!response.ok||!String(response.headers.get('content-type')||'').includes('text/html'))return response;
  const headers=new Headers(response.headers);
  headers.delete('content-length');
  headers.delete('content-encoding');
  headers.set('cache-control','no-store, no-cache, must-revalidate, max-age=0');
  ...
  let body=await response.text();
  body=body
    .replace(/<script\b[^>]*\bsrc=["'][^"']*\/assets\/gallery-bootstrap\.js[^"']*["'][^>]*>\s*<\/script>/gi,'')
    .replace(/<script\b[^>]*\bsrc=["'][^"']*\/assets\/gallery-brand-safety\.js[^"']*["'][^>]*>\s*<\/script>/gi,'');
...
export default{
  async fetch(request,env,ctx){
    ...
    if(['GET','HEAD'].includes(request.method))response=await patchIndexShowcase(response,path,request,env);
