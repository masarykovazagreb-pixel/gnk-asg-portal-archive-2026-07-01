...
async function correctNewsStatus(response){
  ...
  try{
    ...
    const corrected={
      ...
      configuredNewsSources:18,
      sourceMix:SOURCE_MIX,
      activeNewsLimit:100,
      archivePruneAt:1000,
      archiveDeleteCount:500,
      archiveRetainAfterPrune:500,
      newsRuntime:'GNK_ASG_NEWS_LIFECYCLE_V13_20260626'
    };
...
export default{
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&['/data/news-automation-status.json','/data/deployment-status.json'].includes(path))return correctNewsStatus(response);
    const headers=new Headers(response.headers);
    headers.set('x-gnk-asg-admin-hub-v22',VERSION);
