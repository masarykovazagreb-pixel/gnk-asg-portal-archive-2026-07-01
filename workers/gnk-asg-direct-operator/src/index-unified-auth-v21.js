...
async function enhance(request,response){if(request.method!=='GET'||![200,401,403,503].includes(response.status))return response;const type=String(response.headers.get('content-type')||'').toLowerCase();if(!type.includes('text/html'))return response;try{let html=await response.text();html=html.replace(/<script[^>]+public-unified-menu-v6\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+public-contrast-hardening-v1\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-editorial-order-v6\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+mail-studio-ui-v28\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+public-unified-menu-v5\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-editorial-order-v5\.js[^>]*><\/script>/gi,'');const route=pathOf(request),scripts=`${isIndex(route)?EDITORIAL:''}${isMailStudio(route)?MAIL_STUDIO:''}${CONTRAST}${MENU}`;html=html.includes('</body>')?html.replace('</body>',`${scripts}</body>`):`${html}${scripts}`;const headers=new Headers(response.headers);headers.delete('content-length');headers.delete('content-encoding');headers.delete('location');headers.set('content-type','text/html; charset=utf-8');headers.set('x-gnk-public-runtime',VERSION);headers.set('x-gnk-unified-menu-current','full-v6-workers');headers.set('x-gnk-contrast','hardened-v1');headers.set('x-gnk-index-editorial',isIndex(route)?'v6-news-100':'not-applicable');headers.set('x-gnk-mail-transport',CONTACT_STUDIO_VERSION);if(isMailStudio(route))headers.set('x-gnk-mail-studio-ui','v28-large-composer');return new Response(html,{status:response.status,statusText:response.statusText,headers})}catch{return response}}
export default{
