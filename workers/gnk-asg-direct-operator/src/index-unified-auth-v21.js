...
const MENU='<script defer src="/assets/public-unified-menu-v6.js?v=20260728-restaurants-menu-v1"></script>';
...
async function enhance(request,response){if(request.method!=='GET'||![200,401,403,503].includes(response.status))return response;const type=String(response.headers.get('content-type')||'').toLowerCase();if(!type.includes('text/html'))return response;try{const route=pathOf(request);let html=normalizeHtml(await response.text(),route);html=html.replace(/<script[^>]+public-unified-menu-v6\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+public-contrast-hardening-v1\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-editorial-order-v6\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-editorial-cleanup-v1\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+mail-studio-ui-v28\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+public-unified-menu-v5\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-editorial-order-v5\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-editorial-order-v1\.js[^>]*><\/script>/gi,'');const scripts=`${isMailStudio(route)?MAIL_STUDIO:''}${isIndex(route)?EDITORIAL:''}${MENU}${CONTRAST}`;html=html.includes('</body>')?html.replace('</body>',`${scripts}</body>`):`${html}${scripts}`;const headers=new Headers(response.headers);headers.delete('content-length');headers.delete('content-encoding');headers.delete('location');headers.set('content-type','text/html; charset=utf-8');headers.set('x-gnk-public-runtime',VERSION);headers.set('x-gnk-unified-menu-current','v6-restaurants-enabled');headers.set('x-gnk-contrast','hardened-v1');headers.set('x-gnk-index-editorial',isIndex(route)?'v6-news-100':'not-applicable');headers.set('x-gnk-mail-transport',CONTACT_STUDIO_VERSION);headers.set('x-gnk-html-normalization','legacy-assets-canonical-logo-a11y-news-v2');headers.set('x-gnk-zero-warning-normalization','zero-warning-canonical-logo-a11y-news-v3');headers.set('x-gnk-logo-standard','64x66');if(isMailStudio(route))headers.set('x-gnk-mail-studio-ui','v28-large-composer');return new Response(html,{status:response.status,statusText:response.statusText,headers})}catch{return response}}
export default{
 async fetch(request,env,ctx){const protectedData=await approvalData(request,env,ctx);if(protectedData)return protectedData;const path=pathOf(request);if(handlesContactStudio(path))return handleContactStudio(request,env,ctx,app);const article=await directArticle(request,env);return enhance(request,article||await app.fetch(request,env,ctx))},
