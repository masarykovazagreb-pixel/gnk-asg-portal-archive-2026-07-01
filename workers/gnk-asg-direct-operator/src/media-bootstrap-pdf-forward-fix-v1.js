...
export const VERSION=`GNK_ASG_MEDIA_BOOTSTRAP_PDF_FORWARD_FIX_V3_20260704_ARCHIVE_${SIGNATURE_VERSION}`;
...
export async function handleMediaBootstrapPdfForwardFix(message,env){
  ...
  if(message?.canBeForwarded!==false&&typeof message?.forward==='function'){
    const archiveHeaders=new Headers();
    archiveHeaders.set('X-GNK-ASG-Archive-Copy','media-bootstrap-inbound');
    archiveHeaders.set('X-GNK-ASG-Original-Recipient',ALLOWED_TO);
    archiveHeaders.set('X-GNK-ASG-Archive-Contract',VERSION);
    try{await message.forward(MANDATORY_BCC,archiveHeaders);}catch(error){console.error('media-bootstrap-archive',error);}
  }
