...
export const VERSION=`GNK_ASG_MAIL_STUDIO_EXTENSION_V15_20260706_DIRECT_MANUAL_OUTBOUND_${IDENTITY_VERSION}_${MAIL_SYNC_VERSION}_${base.VERSION}`;
...
function stamp(response){if(!response)return response;const headers=new Headers(response.headers);headers.set('x-gnk-asg-mail-studio-extension',VERSION);headers.set('x-gnk-asg-mail-sync',MAIL_SYNC_VERSION);headers.set('x-gnk-asg-mail-identity-context',IDENTITY_VERSION);headers.set('x-gnk-asg-manual-outbound-env','direct');return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}
...
async function injectUi(request,response){const path=pathOf(request),type=String(response?.headers?.get('content-type')||'').toLowerCase();if(!(path==='/mail-studio'||path.startsWith('/mail-studio/')||path==='/mail-studio-pro'||path.startsWith('/mail-studio-pro/')||path==='/admin-center')||!type.includes('text/html'))return response;let html=await response.text();const tag=`<script id="gnk-mail-sync-center-ui" defer src="${UI_SCRIPT}"></script>`;if(!html.includes('gnk-mail-sync-center-ui'))html=/<\/body>/i.test(html)?html.replace(/<\/body>/i,`${tag}</body>`):`${html}${tag}`;const headers=new Headers(response.headers);headers.delete('content-length');headers.delete('content-encoding');headers.delete('etag');headers.set('cache-control','no-store, no-cache, must-revalidate, max-age=0');return new Response(html,{status:response.status,statusText:response.statusText,headers});}
...
export async function handleMailStudioInbound(message,env,ctx){return base.handleMailStudioInbound(message,withMailIdentityContext(env,'inbound'),ctx);}
