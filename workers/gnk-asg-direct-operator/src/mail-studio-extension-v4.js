...
function stamp(response){if(!response)return response;const headers=new Headers(response.headers);headers.set('x-gnk-asg-mail-studio-extension',VERSION);headers.set('x-gnk-asg-mail-sync',MAIL_SYNC_VERSION);headers.set('x-gnk-asg-mail-identity-context',IDENTITY_VERSION);return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}
...
export async function handleMailStudioExtension(request,env,ctx,app){if(pathOf(request).startsWith(PUBLIC_PREFIX))return stamp(await handleMailSyncCenter(mappedRequest(request),env));return stamp(await base.handleMailStudioExtension(request,withMailIdentityContext(env,'outbound'),ctx,app));}
