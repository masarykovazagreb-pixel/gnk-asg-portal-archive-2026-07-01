...
async function sendGate(request,env){
  ...
  const scheduledMinInterval=int(env.MAIL_SCHEDULED_MIN_INTERVAL_SECONDS,600,0,86400);
  try{
    ...
    if(scheduledId&&scheduledMinInterval>0&&lastSentAt){
      const elapsed=Math.floor((Date.now()-Date.parse(lastSentAt))/1000);
      if(Number.isFinite(elapsed)&&elapsed<scheduledMinInterval){
        const retryAfterSeconds=scheduledMinInterval-elapsed;
        return json({ok:false,error:'scheduled_send_interval_active',message:`Zakazana poruka čeka još ${Math.ceil(retryAfterSeconds/60)} min radi kontroliranog razmaka.`,retryAfterSeconds,minIntervalSeconds:scheduledMinInterval,used,limit:dailyLimit,scheduledId},429);
      }
    }
    return null;
  }catch(error){console.error('mail-studio-send-gate',error);return null;}
}
...
function senderEmail(payload){const from=payload?.from;return clean(from&&typeof from==='object'?from.email:from).toLowerCase();}
function insertBeforeLast(source,needle,insertion){const index=source.toLowerCase().lastIndexOf(String(needle||'').toLowerCase());return index<0?`${source.trim()}\n${insertion}`:`${source.slice(0,index)}${insertion}\n${source.slice(index)}`;}
...
function withCityEmail(env,city){
  ...
  return new Proxy(env,{get(target,property,receiver){if(property==='EMAIL')return{send(payload){return binding.send.call(binding,rewritePayload(payload,city));}};return Reflect.get(target,property,receiver);}});
}
...
function withCityForward(message,city){
  ...
  return new Proxy(message,{get(target,property,receiver){
    if(property==='forward')return async(destination,headers)=>{const next=new Headers(headers||{});next.set('X-GNK-ASG-City',city.name);next.set('X-GNK-ASG-Global-Centre',city.name);return target.forward(destination,next);};
...
async function patchJsonResponse(response,city){
  ...
  try{
    ...
    const headers=new Headers(response.headers);headers.delete('content-length');headers.set('x-gnk-asg-mail-studio-extension',VERSION);if(providerCode)headers.set('x-gnk-asg-email-error-code',providerCode);
...
export async function handleMailStudioExtension(request,env,ctx,app){
  const blocked=await sendGate(request,env);if(blocked)return blocked;
...
export async function patchMailStudioResponse(request,response){
  ...
  const headers=new Headers(patched.headers);headers.set('x-gnk-asg-mail-studio-extension',VERSION);
