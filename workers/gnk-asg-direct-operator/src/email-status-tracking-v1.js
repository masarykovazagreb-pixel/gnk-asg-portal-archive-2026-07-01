...
export async function ensureEmailStatusSchema(env){
 ...
 for(const col of ['last_open_ip TEXT','last_open_user_agent TEXT','last_open_device TEXT']){
...
async function recordOpen(env,id,request){
 ...
 const ip=clean(request&&request.headers&&request.headers.get('CF-Connecting-IP'))||null;
 const ua=clean(request&&request.headers&&request.headers.get('User-Agent'))||null;
 const device=deviceLabel(ua);
 await db.prepare(`UPDATE email_status_records SET current_status=?,first_opened_at=COALESCE(first_opened_at,?),last_opened_at=?,open_count=open_count+1,last_open_ip=?,last_open_user_agent=?,last_open_device=?,updated_at=? WHERE tracking_id=?`).bind(next,stamp,stamp,ip,ua,device,stamp,id).run();
 return true;
}
...
async function api(url,opt){const r=await fetch(url,{credentials:'same-origin',cache:'no-store',...opt});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||d.error||('HTTP '+r.status));return d}async function load(){const q=new URLSearchParams({limit:'300',status:document.querySelector('#status').value,source:document.querySelector('#source').value,search:document.querySelector('#search').value});const d=await api('${API_PREFIX}/records?'+q);document.querySelector('#stats').innerHTML=Object.entries(d.summary||{}).map(([s,n])=>'<div class="card stat"><span class="badge '+esc(s)+'">'+esc(labels[s]||s)+'</span><b>'+n+'</b></div>').join('');document.querySelector('#rows').innerHTML=(d.items||[]).map(x=>'<tr><td><span class="badge '+esc(x.current_status)+'">'+esc(labels[x.current_status]||x.current_status)+'</span></td><td><b>'+esc(x.recipient)+'</b><div class="muted small">'+esc(x.sender||'')+'</div></td><td>'+esc(x.subject||'—')+'</td><td>'+esc(x.source_system)+'</td><td>'+fmt(x.accepted_at)+'</td><td>'+fmt(x.delivered_at)+'</td><td>'+fmt(x.first_opened_at)+'</td><td>'+Number(x.open_count||0)+'</td><td class="small">'+esc(x.last_open_device||'—')+'<div class="muted small">'+esc(x.last_open_ip||'')+'</div></td><td class="small '+(x.error_detail?'error':'')+'">'+esc(x.error_detail||x.provider_message_id||'—')+'</td></tr>').join('')||'<tr><td colspan="10">Nema zapisa.</td></tr>';document.querySelector('#message').textContent='Ukupno: '+d.total+(d.sync?.last_completed_at?' · zadnja sinkronizacija: '+fmt(d.sync.last_completed_at):'')}
