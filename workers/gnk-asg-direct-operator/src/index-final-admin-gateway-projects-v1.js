...
import {handleMediaDelivery,VERSION as DELIVERY_VERSION,INTERNAL_TEST_PATH} from './media-outreach-delivery-v5.js';
...
import {withEmbeddedMediaAutoReplyLogo,VERSION as INLINE_LOGO_VERSION} from './media-auto-reply-inline-logo-v1.js';
import {isMediaInboxMessage,withoutLegacyMediaAcknowledgement,sendGlobalMediaAutoReply,VERSION as GLOBAL_REPLY_VERSION} from './media-global-auto-reply-v1.js';
...
function stamp(response){
  ...
  headers.set('x-gnk-asg-media-launch',LAUNCH_VERSION);
  headers.set('x-gnk-asg-media-final-message',FINAL_MESSAGE_VERSION);
  ...
  headers.set('x-gnk-asg-media-inline-logo',INLINE_LOGO_VERSION);
  headers.set('x-gnk-asg-media-global-reply',GLOBAL_REPLY_VERSION);
...
export default{
  ...
  async email(message,env,ctx){
    ...
    if(typeof app.email==='function'){
      if(isMediaInboxMessage(message)){
        await app.email(message,withoutLegacyMediaAcknowledgement(signed),ctx);
        return sendGlobalMediaAutoReply(message,signed);
      }
      return app.email(message,signed,ctx);
    }
  }
};
