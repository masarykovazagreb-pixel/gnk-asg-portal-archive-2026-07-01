...
export const VERSION=`GNK_ASG_FINAL_MEDIA_BOOTSTRAP_HTML_FORWARD_V1_TO_${BASE_VERSION}`;
...
function stamp(response){const headers=new Headers(response.headers);headers.set('x-gnk-asg-final-media-fresh',VERSION);headers.set('x-gnk-asg-media-projects',PROJECTS_VERSION);headers.set('x-gnk-asg-media-delivery',DELIVERY_VERSION);headers.set('x-gnk-asg-media-fresh',FRESH_VERSION);headers.set('x-gnk-asg-media-bootstrap',BOOTSTRAP_VERSION);headers.set('x-gnk-asg-media-bootstrap-html',BOOTSTRAP_HTML_VERSION);headers.set('x-gnk-asg-email-signature-contract',SIGNATURE_VERSION);headers.set('cache-control','no-store, no-cache, must-revalidate, max-age=0');return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}
async function protectedEndpoint(request,env,ctx,handler){const probe=await app.fetch(request.clone(),env,ctx);if(probe.status===401||probe.status===403)return stamp(probe);const response=await handler(request,env,ctx);return stamp(response||probe);}
export default{async fetch(request,env,ctx){const signed=activeEnv(env),path=pathOf(request);if(path===PUBLIC_MEMORANDUM)return publicMemorandum(request,signed);if(path===INTERNAL_TEST_PATH)return stamp(await handleMediaDelivery(request,signed));if(FRESH_ENDPOINTS.has(path))return protectedEndpoint(request,signed,ctx,handleFreshMediaAdmin);if(DELIVERY_ENDPOINTS.has(path))return protectedEndpoint(request,signed,ctx,handleMediaDelivery);if(PROJECT_ENDPOINTS.has(path))return protectedEndpoint(request,signed,ctx,handleMediaProjects);let response=await app.fetch(request,signed,ctx);if(path==='/data/portal-version.json'&&response.ok&&String(response.headers.get('content-type')||'').includes('application/json')){try{const payload=await response.clone().json(),headers=new Headers(response.headers);response=new Response(JSON.stringify({...payload,finalMediaFresh:VERSION,mediaFresh:FRESH_VERSION,mediaDelivery:DELIVERY_VERSION,mediaProjects:PROJECTS_VERSION,mediaBootstrap:BOOTSTRAP_VERSION,mediaBootstrapHtml:BOOTSTRAP_HTML_VERSION,mediaOutreachBuildLock:false},null,2),{status:response.status,headers});}catch{}}return stamp(response);},async scheduled(event,env,ctx){const signed=activeEnv(env),task=(async()=>{const upstream=typeof app.scheduled==='function'?await app.scheduled(event,signed,ctx):null;const delivery=await processDeliveryQueue(signed).catch(error=>({ok:false,error:String(error?.message||error)}));return{upstream,delivery};})();if(ctx?.waitUntil){ctx.waitUntil(task);return;}return task;},async email(message,env,ctx){const signed=activeEnv(env);const htmlBootstrap=await handleMediaBootstrapHtmlForwardEmail(message,signed,ctx);if(htmlBootstrap?.handled)return htmlBootstrap;const bootstrap=await handleMediaBootstrapEmail(message,signed,ctx);if(bootstrap?.handled)return bootstrap;if(typeof app.email==='function')return app.email(message,signed,ctx);}};