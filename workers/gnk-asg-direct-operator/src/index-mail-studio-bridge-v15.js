...
async function profileAuth(request,env,ctx){
  ...
  return new Response(JSON.stringify({ok:false,error:'authentication_required'},null,2),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store','x-gnk-asg-mail-studio-bridge':VERSION,'x-gnk-asg-mail-profile-delivery-test':PROFILE_DELIVERY_VERSION,'x-gnk-asg-manual-mail-service':MANUAL_MAIL_VERSION}});
}
