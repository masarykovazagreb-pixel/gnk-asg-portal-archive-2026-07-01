// Production activation R5: preserve the verified IQ200 index while routing all private modules through the active admin hub.
import app from './index-admin-hub-v28-news-no-fallback.js';
import indexApp from './index-portal-final-v13.js';

export const VERSION='GNK_ASG_FINAL_GATEWAY_IQ200_20260625_WITH_ADMIN_HUB_V31_20260629';
...
function stamp(response,flow){
  ...
  headers.set('x-gnk-asg-production-entry','GNK_ASG_IQ200_INDEX_ADMIN_HUB_V31');
...
function patchIq200Navigation(html,english){
  ...
  if(!html.includes('gnk-iq200-pdf-admin-lock-r5'))html=html.replace('</body>',lock+'</body>');
  return html;
}
  ...
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&INDEX_PATHS.has(path)){
      ...
      if(response.ok&&isHtml(response)){
        ...
        return new Response(html,{status:response.status,statusText:response.statusText,headers:stamp(response,'IQ200_INDEX_ADMIN_HUB_V31')});
      }
      return new Response(response.body,{status:response.status,statusText:response.statusText,headers:stamp(response,'IQ200_INDEX_ADMIN_HUB_V31')});
    }
    const response=await app.fetch(request,env,ctx);
    return new Response(response.body,{status:response.status,statusText:response.statusText,headers:stamp(response,'ADMIN_HUB_V31_MEDIA_PORTAL')});
  },
