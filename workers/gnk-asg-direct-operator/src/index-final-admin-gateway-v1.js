// Production activation R6: preserve the verified IQ200 index while routing private modules through the isolated English media hub.
import app from './index-admin-hub-v34-english-media.js';
import indexApp from './index-portal-final-v13.js';

export const VERSION='GNK_ASG_FINAL_GATEWAY_IQ200_20260625_WITH_ADMIN_HUB_V34_ENGLISH_20260629';
...
function stamp(response,flow){
  ...
  headers.set('x-gnk-asg-production-entry','GNK_ASG_IQ200_INDEX_ADMIN_HUB_V34_ENGLISH');
  ...
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&INDEX_PATHS.has(path)){
      ...
      if(response.ok&&isHtml(response)){
        ...
        return new Response(html,{status:response.status,statusText:response.statusText,headers:stamp(response,'IQ200_INDEX_ADMIN_HUB_V34_ENGLISH')});
      }
      return new Response(response.body,{status:response.status,statusText:response.statusText,headers:stamp(response,'IQ200_INDEX_ADMIN_HUB_V34_ENGLISH')});
    }
    const response=await app.fetch(request,env,ctx);
    return new Response(response.body,{status:response.status,statusText:response.statusText,headers:stamp(response,'ADMIN_HUB_V34_ENGLISH_MEDIA_PORTAL')});
  },
