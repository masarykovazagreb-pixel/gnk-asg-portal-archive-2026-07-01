...
export const VERSION='GNK_ASG_FINAL_GATEWAY_IQ200_20260625_WITH_ADMIN_HUB_V32_20260629';
...
function stamp(response,flow){
  ...
  headers.set('x-gnk-asg-production-entry','GNK_ASG_IQ200_INDEX_ADMIN_HUB_V32');
  ...
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&INDEX_PATHS.has(path)){
      ...
      if(response.ok&&isHtml(response)){
        ...
        return new Response(html,{status:response.status,statusText:response.statusText,headers:stamp(response,'IQ200_INDEX_ADMIN_HUB_V32')});
      }
      return new Response(response.body,{status:response.status,statusText:response.statusText,headers:stamp(response,'IQ200_INDEX_ADMIN_HUB_V32')});
    }
    const response=await app.fetch(request,env,ctx);
    return new Response(response.body,{status:response.status,statusText:response.statusText,headers:stamp(response,'ADMIN_HUB_V32_MEDIA_PORTAL')});
  },
