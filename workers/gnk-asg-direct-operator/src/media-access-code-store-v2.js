export const VERSION='GNK_ASG_MEDIA_ACCESS_CODE_STORE_V2_ATOMIC_AUDIT_20260628';
...
function auditStatement(db,eventType,prepared,detail,stamp){return db.prepare(`INSERT INTO media_access_audit(id,event_type,mail_code,email,detail_json,created_at) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),eventType,prepared.mailCode,prepared.email,JSON.stringify(detail),stamp)}
...
export async function prepareAccessCode(env,{mailCode:rawCode,email:rawEmail,queueId='',pdfSha256=''}){
  ...
  const results=await db.batch([
    db.prepare(`UPDATE media_access_codes SET status='REPLACED',revoked_at=?,updated_at=? WHERE mail_code=? AND status='DELIVERY_PENDING'`).bind(stamp,stamp,mailCode),
    db.prepare(`INSERT INTO media_access_codes(id,mail_code,email,code_hash,status,expires_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)`).bind(id,mailCode,email,await codeHash(env,mailCode,email,code),'DELIVERY_PENDING',expiresAt,stamp,stamp),
    auditStatement(db,'access_code_delivery_pending',prepared,{queueId,pdfSha256,version:VERSION},stamp)
  ]);
  if(Number(results?.[1]?.meta?.changes||0)!==1)throw Object.assign(new Error('Pending access code insert failed'),{code:'MEDIA_ACCESS_PREPARE_FAILED'});
  return prepared;
}
...
export async function activateAccessCode(env,prepared,{queueId='',messageId=''}={}){
  ...
  const results=await db.batch([
    db.prepare(`UPDATE media_access_codes SET status='REPLACED',revoked_at=?,updated_at=? WHERE mail_code=? AND id<>? AND status IN ('ACTIVE','DRY_RUN_READY')`).bind(stamp,stamp,prepared.mailCode,prepared.id),
    db.prepare(`UPDATE media_access_codes SET status='ACTIVE',sent_at=?,updated_at=? WHERE id=? AND status='DELIVERY_PENDING'`).bind(stamp,stamp,prepared.id),
    auditStatement(db,'access_code_invitation_sent',prepared,{queueId,messageId,expiresAt:prepared.expiresAt,version:VERSION},stamp)
  ]);
  if(Number(results?.[1]?.meta?.changes||0)!==1)throw Object.assign(new Error('Activation failed'),{code:'MEDIA_ACCESS_ACTIVATION_FAILED'});
}
...
export async function failAccessCode(env,prepared,error,{queueId='',messageId=''}={}){
  ...
  const db=await schema(env),stamp=now();
  const results=await db.batch([
    db.prepare(`UPDATE media_access_codes SET status='DELIVERY_FAILED',revoked_at=?,updated_at=? WHERE id=? AND status='DELIVERY_PENDING'`).bind(stamp,stamp,prepared.id),
    auditStatement(db,'access_code_delivery_failed',prepared,{queueId,messageId,errorCode:clean(error?.code),error:String(error?.message||error).slice(0,240),version:VERSION},stamp)
  ]);
  return{changed:Number(results?.[0]?.meta?.changes||0)};
}
