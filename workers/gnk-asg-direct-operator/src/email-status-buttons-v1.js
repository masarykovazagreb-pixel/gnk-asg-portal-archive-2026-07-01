...
function script(app){return `<script id="gnk-email-tools-hub-v4" src="${SCRIPT}" data-gnk-email-tools="1" data-source="${attr(app.source)}" data-app="${attr(app.name)}" defer></script>`;}
export async function addEmailStatusButtons(request,response){
 if(!response||!['GET','HEAD'].includes(request.method))return response;
 const app=appFor(pathOf(request)),type=String(response.headers?.get?.('content-type')||'').toLowerCase();
 if(!app||!type.includes('text/html'))return response;
 let html=await response.text();
 if(!html.includes('gnk-email-tools-hub-v4'))html=/<\/body>/i.test(html)?html.replace(/<\/body>/i,`${script(app)}</body>`):`${html}${script(app)}`;
 const headers=new Headers(response.headers);headers.delete('content-length');headers.delete('content-encoding');headers.set('cache-control','no-store, no-cache, must-revalidate, max-age=0');headers.set('x-gnk-asg-email-status-buttons',VERSION);
