...
const DIRECT_HTML=new Map([['/','/index.html'],['/en','/en/index.html'],['/contact','/contact/index.html'],['/en/contact','/en/contact/index.html']]);
...
async function directHtml(request,env,path){
  ...
  const assetPath=DIRECT_HTML.get(path),assetUrl=new URL(assetPath,request.url);
  const response=await env.ASSETS.fetch(new Request(assetUrl.toString(),{method:'GET',headers:{accept:'text/html'}}));
  if(!response.ok)return null;
  const headers=htmlHeaders(new Headers(response.headers));headers.set('x-gnk-asg-active-entrypoint',ENTRYPOINT);headers.set('x-gnk-asg-static-source',assetPath);headers.set('x-gnk-asg-hotfix-version',VERSION);
  let body=await response.text();
  if(INDEX_PATHS.has(path)){
    if(!body.includes('index-priority-v1.css'))body=body.replace('</head>',`${INDEX_STYLE}</head>`);
    if(!body.includes('index-priority-v1.js'))body=body.replace('</body>',`${INDEX_SCRIPT}</body>`);
    headers.set('x-gnk-asg-index-menu','LOCKED_REDUCED_PUBLIC_MENU');headers.set('x-gnk-asg-index-cycle','FULL_AUTO_LOOP_WITH_COUNTDOWN');headers.set('x-gnk-asg-index-network','MAP_AND_CITY_DIRECTORY_RETAINED');
  }
  if(path==='/contact'||path==='/en/contact')headers.set('x-gnk-asg-contact-ui','INDEX_ALIGNED_V5_FUNCTIONAL');
...
async function patchMediaCommandStatus(response,path){
  ...
  try{
    const payload=await response.json(),headers=noStore(new Headers(response.headers));headers.set('x-gnk-asg-media-command-sending','LOCKED');headers.set('x-gnk-asg-email-routing-verification','UNVERIFIED_EXTERNAL_CONFIGURATION');headers.set('x-gnk-asg-contact-seed','INCOMPLETE_22_OF_42');
