...
async function patchMapFilm(response,path,request){
  if(response.headers.get('x-gnk-asg-index-template'))return response;
  ...
  body=body.replace(/<link[^>]+index-mobile-code-launch-v1\.css[^>]*>/gi,'').replace(/<a class="gnk-activation__code-open"[\s\S]*?<\/a>/gi,'').replace(/<span class="gnk-activation__label">THE CODE<\/span>/gi,'');
