...
export async function handleContactSubmit(request,env){
  ...
  await Promise.all([
    kv.put(`contact:${caseId}`,JSON.stringify(record,null,2)),
    kv.put('contact:index',JSON.stringify([summary,...(Array.isArray(index)?index:[]).filter(item=>item?.caseId!==caseId)].slice(0,500),null,2))
  ]);

  let delivery={sent:false,error:''};
  if(env.EMAIL?.send){
    const payload={
      to:mailboxAddress,from:{email:'contact@gnk-asg.hr',name:'GNK ASG Contact'},replyTo:email,
      subject:`[${caseId}] ${subject}`,
      text:[`Evidencijski broj: ${caseId}`,`Odjel: ${mailboxKey}`,`Ime / naziv: ${name}`,`E-mail: ${email}`,`Telefon: ${phone||'—'}`,'',message].join('\n'),
    ...
    try{const result=await env.EMAIL.send(payload);delivery={sent:true,messageId:clean(result?.messageId)};}
    catch(error){delivery={sent:false,error:String(error?.message||error).slice(0,300)};}
  }
