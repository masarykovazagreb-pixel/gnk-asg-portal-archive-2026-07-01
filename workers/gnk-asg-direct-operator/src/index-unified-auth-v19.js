...
const RELEASE='<script defer src="/assets/release-completion-v1.js?v=20260713-index-final-v6"></script><script defer src="/assets/index-data-resilience-v1.js?v=20260713-resilience-v1"></script>';
...
async function finalize(request,response){
  ...
  try{
    ...
    html=html.replace(/<script[^>]+release-completion-v1\.js[^>]*><\/script>/gi,'').replace(/<script[^>]+index-data-resilience-v1\.js[^>]*><\/script>/gi,'');
    html=html.includes('</body>')?html.replace('</body>',`${RELEASE}</body>`):`${html}${RELEASE}`;
    const headers=new Headers(response.headers);
    headers.delete('content-length');headers.delete('content-encoding');
    headers.set('content-type','text/html; charset=utf-8');
    headers.set('x-gnk-index-release','v6-final-resilient');
