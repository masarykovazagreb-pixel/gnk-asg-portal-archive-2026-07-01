export const VERSION='GNK_ASG_MEDIA_PUBLIC_REGISTRATION_V2_20260710_PBKDF2_RATE_LIMIT_ADMIN_REVIEW';
...
const keyRate=(scope,id)=>`media:ratelimit:${scope}:${id}`;
const keyIndex=()=>`media:account-index`;
...
function ipOf(request){return clean(request.headers.get('cf-connecting-ip')||request.headers.get('x-forwarded-for')||'unknown').split(',')[0].slice(0,80)}
...
async function rateLimit(request,store,scope,limit=20,ttl=900){const id=await hmac('media-rate-limit',`${scope}:${ipOf(request)}`);const key=keyRate(scope,id);const state=await readJson(store,key,{count:0,first:now()});state.count=Number(state.count||0)+1;state.last=now();await writeJson(store,key,state,{expirationTtl:ttl});return state.count<=limit;}
async function passwordHash(password,salt,iterations=120000){const key=await crypto.subtle.importKey('raw',enc.encode(String(password||'')),'PBKDF2',false,['deriveBits']);const bits=await crypto.subtle.deriveBits({name:'PBKDF2',salt:rb64(salt),iterations,hash:'SHA-256'},key,256);return b64(bits)}
...
function publicAccount(account){return {id:account.id,username:account.username,email:account.email,outlet:account.outlet,country:account.country,website:account.website,status:account.status||'DRAFT',createdAt:account.createdAt,updatedAt:account.updatedAt,lastLoginAt:account.lastLoginAt||null};}
function validUsername(username){return /^[a-z0-9._-]{4,40}$/.test(username);}
async function addToIndex(store,account){const index=await readJson(store,keyIndex(),[]);const next=[account.id,...index.filter(id=>id!==account.id)].slice(0,1000);await writeJson(store,keyIndex(),next);}

async function register(request,env){
  const store=kv(env);if(!store)return json({ok:false,error:'kv_not_configured'},503);
  if(!await rateLimit(request,store,'register',10,900))return json({ok:false,error:'rate_limited'},429);
  ...
  const id=crypto.randomUUID(),salt=b64(crypto.getRandomValues(new Uint8Array(16))),iterations=120000,account={id,username,email,outlet,country,website,salt,iterations,passwordHash:await passwordHash(password,salt,iterations),status:'DRAFT',createdAt:now(),updatedAt:now(),flow:'public-media-user-password'};
  await writeJson(store,keyAccount(id),account);await store.put(keyUsername(username),id);await addToIndex(store,account);
...
async function login(request,env){
  const store=kv(env);if(!store)return json({ok:false,error:'kv_not_configured'},503);
  if(!await rateLimit(request,store,'login',30,900))return json({ok:false,error:'rate_limited'},429);
  ...
  const valid=eq(await passwordHash(password,account.salt,Number(account.iterations||120000)),account.passwordHash);if(!valid)return json({ok:false,error:'invalid_credentials'},401);
...
async function adminList(request,env){const store=kv(env);if(!store)return json({ok:false,error:'kv_not_configured'},503);const ids=await readJson(store,keyIndex(),[]);const accounts=[];for(const id of ids.slice(0,200)){const account=await readJson(store,keyAccount(id),null);if(account)accounts.push(publicAccount(account));}return json({ok:true,version:VERSION,total:accounts.length,accounts});}
async function adminRead(request,env){const store=kv(env);if(!store)return json({ok:false,error:'kv_not_configured'},503);const id=new URL(request.url).searchParams.get('id');if(!id)return json({ok:false,error:'missing_id'},400);const account=await readJson(store,keyAccount(id),null);if(!account)return json({ok:false,error:'not_found'},404);const draft=await readJson(store,keyDraft(id),null);const documents=await readJson(store,keyDocs(id),[]);return json({ok:true,account:publicAccount(account),draft,documents});}
...
export function isAdminMediaRegistrationApi(path){return path==='/api/media-registration-admin'||path.startsWith('/api/media-registration-admin/');}
...
export async function handleAdminMediaRegistration(request,env){
  const path=new URL(request.url).pathname.replace(/\/+$/,'')||'/';
  if(request.method==='GET'&&path==='/api/media-registration-admin/list')return adminList(request,env);
  if(request.method==='GET'&&path==='/api/media-registration-admin/read')return adminRead(request,env);
  return json({ok:false,error:'not_found',path},404);
}
