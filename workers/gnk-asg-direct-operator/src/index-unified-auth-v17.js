...
export const VERSION=`GNK_ASG_UNIFIED_AUTH_V48_20260712_EMAIL_LOGO_${EMAIL_LOGO_VERSION}_${MAIL_AUTOREPLY_VERSION}_${NEWS_AUTO_PUBLICATION_VERSION}_${BASE_VERSION}`;
...
async function injectGlobalAssets(request,response){
  ...
  try{
    ...
    const scripts=[],path=pathOf(request);
    ...
    if(isIndexPath(path)&&!html.includes('index-live-hub-v1.css')){const style=`<link rel="stylesheet" href="${INDEX_HUB_STYLE}">`;html=html.includes('</head>')?html.replace('</head>',`${style}</head>`):`${style}${html}`;}
    if(scripts.length){const bundle=scripts.join('');html=html.includes('</body>')?html.replace('</body>',`${bundle}</body>`):`${html}${bundle}`;}
    const headers=new Headers(response.headers);headers.delete('content-length');headers.delete('content-encoding');headers.set('content-type','text/html; charset=utf-8');headers.set('x-gnk-global-floating-menu','admin-first-bilingual');
