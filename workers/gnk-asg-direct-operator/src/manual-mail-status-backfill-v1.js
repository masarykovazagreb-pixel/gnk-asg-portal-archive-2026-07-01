...
export async function backfillManualMailStatus(env){
 ...
 let rows=[];
 try{
  const result=await db.prepare(`SELECT id,from_email,to_json,subject,status,provider_json,error_code,error_message,created_at,sent_at FROM manual_mail_messages WHERE datetime(COALESCE(sent_at,created_at))>=datetime('now','-30 days') ORDER BY datetime(COALESCE(sent_at,created_at)) DESC LIMIT 1000`).all();
  rows=result.results||[];
 }catch(error){
  return{ok:false,reason:'manual_audit_unavailable',message:String(error?.message||error).slice(0,300),version:VERSION};
 }
 const statements=[];
 for(const row of rows){
  const recipients=parse(row.to_json,[]),list=Array.isArray(recipients)?recipients.map(emailOf).filter(Boolean):[];
