...
async function duplicateReason(message,env){const kv=store(env),key=await messageIdKey(message);if(!key||!kv)return'';try{return await kv.get(key)?'duplicate_message_id':''}catch{return''}}
async function markMessageId(message,env){const kv=store(env),key=await messageIdKey(message);if(!key||!kv)return false;try{await kv.put(key,now(),{expirationTtl:MESSAGE_ID_TTL});return true}catch{return false}}
