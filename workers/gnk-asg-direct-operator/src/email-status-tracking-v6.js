...
async function listRecords(request,env,backfill){
 ...
 if(search){const term=`%${search}%`;clauses.push(`(LOWER(COALESCE(r.recipient,'')) LIKE ? OR LOWER(COALESCE(r.sender,'')) LIKE ? OR LOWER(COALESCE(r.subject,'')) LIKE ? OR LOWER(COALESCE(r.provider_message_id,'')) LIKE ? OR LOWER(COALESCE(r.source_id,'')) LIKE ?)`);binds.push(term,term,term,term,term);}
 let dateWindow=null;if(date==='today'){dateWindow=emailStatusDayWindow(new Date(),DEFAULT_TIME_ZONE);clauses.push(`datetime(COALESCE(r.accepted_at,r.created_at,r.updated_at))>=datetime(?) AND datetime(COALESCE(r.accepted_at,r.created_at,r.updated_at))<datetime(?)`);binds.push(dateWindow.start,dateWindow.end);}
 const where=clauses.length?`WHERE ${clauses.join(' AND ')}`:'';
 const rowsStatement=db.prepare(`SELECT r.*,
  ...
  FROM email_status_records r ${where} ORDER BY datetime(COALESCE(r.last_event_at,r.updated_at,r.created_at)) DESC LIMIT ? OFFSET ?`).bind(...binds,limit,offset);
 const totalStatement=db.prepare(`SELECT COUNT(*) count FROM email_status_records r ${where}`).bind(...binds);
