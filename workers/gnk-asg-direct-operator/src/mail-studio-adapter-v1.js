...
function recipients(value){return (Array.isArray(value)?value:[]).map(email=>({email,name:''}));}
...
function mapManual(row,folder){
  return{
    ...
    to:recipients(row.to),
    cc:recipients(row.cc),
    bcc:recipients(row.bcc),
...
async function saveDraft(env,body){
  ...
  const draft={
    ...
    to:recipients(Array.isArray(body.to)?body.to:String(body.to||'').split(/[;,]+/).map(item=>item.trim()).filter(Boolean)),
    cc:recipients(Array.isArray(body.cc)?body.cc:String(body.cc||'').split(/[;,]+/).map(item=>item.trim()).filter(Boolean)),
    bcc:recipients(Array.isArray(body.bcc)?body.bcc:String(body.bcc||'').split(/[;,]+/).map(item=>item.trim()).filter(Boolean)),
