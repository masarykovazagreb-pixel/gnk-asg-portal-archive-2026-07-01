...
test('direct inbound metadata is stored without consuming MIME content',async()=>{
  ...
  assert.equal(VERSION,'GNK_ASG_MAIL_INBOUND_SERVICE_V1_METADATA_20260628');
});

test('same Message-ID is deduplicated',async()=>{
  const kv=memoryKv();
  const env={GNK_ASG_KV:kv};
  const first=await storeInboundMetadata(message(),env,{receivedAt:'2026-06-28T08:00:00.000Z'});
  const second=await storeInboundMetadata(message(),env,{receivedAt:'2026-06-28T08:00:00.000Z'});
