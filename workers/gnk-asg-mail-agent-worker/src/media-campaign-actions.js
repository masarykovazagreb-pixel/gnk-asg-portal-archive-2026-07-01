...
export function mediaCampaignStatus(campaign={}){
  ...
  const sent=Number(campaign.sent||0)+count('sent');
  const failed=Number(campaign.failed||0)+count('failed');
  const remaining=count('remaining');
  return {id:campaign.id,status:campaign.status,total:Number(campaign.total||queue.length),sent,failed,remaining,rateLimitPerMinute:MEDIA_CAMPAIGN_LIMITS.maxSendPerMinute};
}
