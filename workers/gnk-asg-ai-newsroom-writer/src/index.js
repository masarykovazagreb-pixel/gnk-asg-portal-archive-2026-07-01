...
const NEWS_SOURCE_URL='https://gnk-asg.hr/api/public-news-feed';
const DAILY_NEWS=10;
...
async function runDailyNewsroom(env){
 ...
 try{
  authHeaders(env);
  const topics=await pickTopics(),scheduledFor=targetNine(),batchId=`daily-10-3:${scheduledFor.slice(0,10)}`,drafts=[];result.scheduledFor=scheduledFor;result.batchId=batchId;
  for(let index=0;index<topics.length;index++){
   const kind=index<DAILY_NEWS?'news':'commentary';
   try{const content=await writeOriginal(topics[index],kind,env);drafts.push({topic:topics[index],kind,content,index})}
  ...
  if(drafts.length!==DAILY_TOTAL){result.errors.push({error:'daily_10_3_generation_incomplete_no_enqueue'});return result}
  for(const draft of drafts){
   try{const entry=await enqueue(draft.topic,draft.kind,draft.content,scheduledFor,env,batchId,draft.index);result.prepared++;if(draft.kind==='news')result.news++;else result.commentaries++;if(entry.post.status==='queued')result.queued++;else result.manualReview++}
   catch(error){result.errors.push({index:draft.index,kind:draft.kind,title:clean(draft.topic?.title).slice(0,160),error:clean(error?.message).slice(0,200)})}
  }
  result.ok=result.prepared===DAILY_TOTAL&&result.news===DAILY_NEWS&&result.commentaries===DAILY_COMMENTARIES&&result.queued===DAILY_TOTAL;
  if(!result.ok)result.errors.push({error:'daily_10_3_batch_incomplete_quarantined'});
