...
const manifest = {
  ...
  days,
  files: [],
  totals: { events: 0, comments: 0, meetings: 0, tasks: 0, drafts: 0, draftsRemovedByDailyLimits: 0 }
};
...
for (let offset = 0; offset < days; offset += 1) {
  ...
  const cycle = applyDailyLimits(generated, operatingModel.dailyLimits);
  const cycleResult = validateCycle(cycle);
  const limitResult = validateDailyLimits(cycle, operatingModel.dailyLimits);
  const errors = [...cycleResult.errors, ...limitResult.errors];
