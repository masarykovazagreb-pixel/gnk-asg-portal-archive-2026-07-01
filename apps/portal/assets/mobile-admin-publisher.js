(() => {
  ...
  function classify(text, lang) {
    ...
    const rules = [
      {category:'BPP i fintech',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/platforme/bpp/',relatedEn:'/en/platforms/bpp/',keywords:['bpp','bitcoin payment processor','merchant','platna infrastruktura','payment processor','fintech']},
      {category:'Tržišta',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/trzista/',relatedEn:'/markets/',keywords:['bitcoin','zlato','brent','nafta','burza','trziste','tržište','euro','dolar','stablecoin','digitalna imovina']},
      {category:'AI i tehnologija',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/assistant/',relatedEn:'/en/assistant/',keywords:['ai','umjetna inteligencija','cloud','softver','software','api','automatizacija','kiberneticka','kibernetička','digitalna transformacija']},
      {category:'Sport i ekonomija',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/#grupa',relatedEn:'/en/#grupa',keywords:['sport','stadion','nogomet','kosarka','košarka','klub','igrac','igrač','sportska infrastruktura']},
      {category:'Mediji i komunikacija',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/media-kit/',relatedEn:'/media-kit/',keywords:['mediji','priopcenje','priopćenje','intervju','press','komunikacija','novinari']},
      {category:'Korporativno upravljanje',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/#o-nama',relatedEn:'/en/#o-nama',keywords:['upravljanje','revizija','kapital','imovina','direktor','nermin sefic','nermin sefić','gnk asg','gnk dinamo ltd']}
    ];
    let best = {category:'Poslovanje',routeHr:'/objave/',routeEn:'/publications/',relatedHr:'/#o-nama',relatedEn:'/en/#o-nama',score:0};
    for (const rule of rules) {
      const score = rule.keywords.reduce((total, keyword) => total + (value.includes(normalize(keyword)) ? 3 : 0), 0);
    ...
    return {
      ...
      reason:best.score ? `Prepoznate ključne teme za rubriku ${best.category}.` : 'Nije pronađena uža tema pa je odabrana opća poslovna rubrika.'
    };
  }
  ...
  function renderShell() {
    ...
    panel.innerHTML = `
      <div class="mqp-auth"><span id="mqpTokenBadge" class="mqp-badge locked">TOKEN NIJE POTVRĐEN</span><span id="mqpAuthText">Objava i upload zahtijevaju valjan operator token.</span></div>
      <div class="mqp-grid">
        <div>
          ...
          <label>Jezik</label><select id="mqpLang"><option value="hr">HR</option><option value="en">EN</option></select>
          <label>Naslov</label><input id="mqpTitle" maxlength="180" placeholder="Naslov objave">
          <label>Kratka bilješka ili cijeli tekst</label><textarea id="mqpText" placeholder="Napišite što se dogodilo, gdje, kada i zašto je važno."></textarea>
          <div id="mqpWordCount" class="mqp-count">0 riječi</div>
          <label>Izvor URL – opcionalno</label><input id="mqpSource" type="url" placeholder="https://...">
        </div>
        <div>
          <label>Predložena rubrika</label><input id="mqpCategory" readonly>
          <div id="mqpRoute" class="mqp-route"></div>
          <label>Sažetak</label><textarea id="mqpSummary" placeholder="Sustav će pripremiti sažetak."></textarea>
          <label>Slika URL nakon uploada</label><input id="mqpImageUrl" readonly>
          <label>Ključne riječi</label><input id="mqpKeywords" placeholder="GNK ASG, ...">
  ...
  function status(text) { if (q('mqpStatus')) q('mqpStatus').textContent = text; }
  ...
  function updateGuard() {
    ...
    if (!badge) return;
    badge.textContent = verified ? 'TOKEN POTVRĐEN' : token() ? 'TOKEN NIJE POTVRĐEN' : 'TOKEN NEDOSTAJE';
    badge.className = `mqp-badge ${verified ? 'ready' : 'locked'}`;
    ['mqpAiPrepare','mqpCheck','mqpPublish'].forEach(id => { if (q(id)) q(id).disabled = !verified; });
  }
  ...
  async function verifyToken() {
    ...
    try {
      const response = await fetch('https://publish.gnk-asg.hr/api/publications/status', {headers:auth(),cache:'no-store'});
      verified = response.ok;
      updateGuard();
      status(verified ? 'Operator token je potvrđen.' : `Token nije prihvaćen. HTTP ${response.status}`);
      return verified;
    } catch (error) {
  ...
  async function uploadImage() {
    ...
    const response = await fetch('/operator/media-upload', {method:'POST',headers:auth(),body:form});
  ...
  async function aiPrepare() {
    ...
    try {
      const response = await fetch('/api/ai-assist',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({question:prompt,prompt,task:'mobile-publication',publicOnly:false})});
      const raw = await response.text();
      let data; try { data = JSON.parse(raw); } catch { data = {answer:raw}; }
      const answer = data.answer || data.response || data.text || data.result;
      if (!response.ok || !answer) throw new Error('AI nije vratio tekst.');
      q('mqpText').value = answer;
      if (!q('mqpSummary').value.trim()) q('mqpSummary').value = answer.replace(/\s+/g,' ').slice(0,300);
      updateCount();
      classifyCurrent();
  ...
  function readDrafts() {
    try { const value = JSON.parse(localStorage.getItem(DRAFT_KEY) || '[]'); return Array.isArray(value) ? value : []; } catch { return []; }
  }
  ...
  function payload(dryRun) {
    ...
    return {
      ...
      keywords:q('mqpKeywords').value.split(',').map(item => item.trim()).filter(Boolean),
      seo:{description:summary,canonicalRoute:classification.publicRoute,relatedRoute:classification.relatedRoute},
      mobileCreated:true,
  ...
  async function publish(dryRun) {
    ...
    if (updateCount() < 500) return status('Objava mora imati najmanje 500 riječi. Upotrijebite AI pripremu ili dopunite tekst.');
    try {
      ...
      const response = await fetch(PUBLISH_URL,{method:'POST',headers:{'content-type':'application/json',...auth()},body:JSON.stringify(data)});
      const raw = await response.text();
      let result; try { result = JSON.parse(raw); } catch { result = {raw}; }
      if (!response.ok) throw new Error(result.error || result.validation?.join(', ') || `HTTP ${response.status}`);
      status(dryRun ? `Provjera je prošla. Rubrika: ${data.category}.` : `Objava je prihvaćena. Rubrika: ${data.category}. Cilj: ${data.targetRoute}`);
    } catch (error) {
  ...
  function bind() {
    ...
    q('mqpText').addEventListener('input',() => { updateCount(); classifyCurrent(); });
    ...
    q('clearToken')?.addEventListener('click',() => { verified=false; updateGuard(); });
    window.addEventListener('hashchange',() => { if (location.hash === '#quick-publish') document.querySelector('[data-tab="quick-publish"]')?.click(); });
    classifyCurrent();
    updateCount();
    verifyToken();
  }
