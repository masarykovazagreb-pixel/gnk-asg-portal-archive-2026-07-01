(() => {
  'use strict';
  if (window.__GNK_ASG_ADMIN_SESSION_FALLBACK_V18__) return;
  window.__GNK_ASG_ADMIN_SESSION_FALLBACK_V18__ = true;
  window.__GNK_ASG_ADMIN_SESSION_FALLBACK_V17__ = true;

  const VERSION = 'GNK_ASG_ADMIN_SESSION_FALLBACK_V18_20260626_COOKIE_ONLY';
  const STORAGE_KEYS = [
  ...
  const boot = async () => {
    ...
    if (button && !button.dataset.gnkSessionFallbackV18) {
      button.dataset.gnkSessionFallbackV18 = '1';
