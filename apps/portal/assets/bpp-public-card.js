(() => {
  ...
  const copy = () => english() ? {
    overline:'SOFTWARE SOLUTION · BPP.IS', title:'Bitcoin Payment Processor — bpp.is',
    body:'Advanced crypto payment infrastructure with secure API integration. Official domain: bpp.is.',
    ...
    overline:'PROGRAMSKO RJEŠENJE · BPP.IS', title:'Bitcoin Payment Processor — bpp.is',
    body:'Napredna infrastruktura kripto plaćanja sa sigurnom API integracijom. Službena domena: bpp.is.',
  ...
  function render() {
    ...
    panel.innerHTML = '<div class="bpp-public-copy"><span class="bpp-public-icon" aria-hidden="true">₿</span><div><small>' + t.overline + '</small><h3>' + t.title + '</h3><p>' + t.body + '</p></div></div><div class="bpp-public-meta"><div class="bpp-public-tags">' + t.tags.map(tag => '<span>' + tag + '</span>').join('') + '</div><p class="bpp-public-legal">' + t.note + '</p><div class="bpp-public-actions"><a class="bpp-public-source" href="' + BPP_URL + '" target="_blank" rel="noopener noreferrer">' + t.open + ' ↗</a><button class="bpp-share-btn" type="button">' + t.share + '</button><button class="bpp-copy-btn" type="button">' + t.link + '</button></div></div>';
