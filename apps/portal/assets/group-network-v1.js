(()=>{
  ...
  const createMarkup=(raw,compact,lang,instanceId)=>{
    ...
    return `<section class="gnk-network ${compact?'gnk-network--compact':''}" data-gnk-network-root data-network-instance="${instanceId}">
      <div class="gnk-network__head"><div><div class="gnk-network__eyebrow">${t.eyebrow}</div><h2 class="gnk-network__title">${t.title}</h2></div><div class="gnk-network__meta"><div class="gnk-network__metric"><span>${t.entities}</span><b>${esc(s.entities??45)}</b></div><div class="gnk-network__metric"><span>${t.operating}</span><b>${esc(s.operatingLocations??33)}</b></div><div class="gnk-network__metric"><span>${t.dormant}</span><b>${esc(s.dormantLocations??12)}</b></div></div></div>
