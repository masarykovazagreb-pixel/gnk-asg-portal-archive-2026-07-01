...
if(!type)return;
const FEATURED_COMMENT={
  slug:'nermin-sefic-dinamo-cinjenice-pravo-dogovor',
  type:'komentar',
  section:'Pravo, poduzetništvo i sport',
  title:'Nermin Sefić: Dinamo nije protivnik — činjenice, pravo i otvorena ponuda za dogovor',
  summary:'Privremena mjera nije pravomoćna presuda. Registracija podružnice nije sama po sebi tržišna uporaba žiga, a ponuda od milijun eura pokazuje spremnost na ulaganje, razgraničenje i miran dogovor.',
  publishedAt:'2026-08-05T14:10:00+02:00',
  image:'/assets/people/nermin-sefic/nermin-sefic-01-official-desk-portrait.webp'
};
...
async function run(){
  ...
  const source=type==='komentar'?[FEATURED_COMMENT,...rows]:rows;
  const seen=new Set();
  const items=source.filter(item=>{if(!item.slug||seen.has(item.slug))return false;seen.add(item.slug);return true}).slice(0,24);
  ...
  const cards=items.map((item,index)=>`<a href="/${type==='objava'?'objave':'komentari'}/${encodeURIComponent(item.slug)}/" style="display:${index===0&&type==='komentar'?'grid':'block'};grid-template-columns:${index===0&&type==='komentar'?'minmax(190px,.75fr) 1.25fr':'none'};gap:${index===0&&type==='komentar'?'18px':'0'};padding:${index===0&&type==='komentar'?'0':'16px'};border:${index===0&&type==='komentar'?'2px solid #8c641f':'1px solid #e5e7eb'};border-radius:14px;color:#111827;text-decoration:none;background:#fff;overflow:hidden">${index===0&&type==='komentar'?`<img src="${esc(item.image)}" alt="${esc(item.title)}" style="width:100%;height:100%;min-height:210px;object-fit:cover">`:''}<span style="display:block;padding:${index===0&&type==='komentar'?'22px':'0'}"><small style="font-weight:800;color:#8c641f">${index===0&&type==='komentar'?'ISTAKNUTI KOMENTAR · ':''}${esc(item.section||type)} · ${esc(date(item.publishedAt))}</small><h3 style="margin:8px 0;font-size:${index===0&&type==='komentar'?'24px':'18px'};line-height:1.3">${esc(item.title)}</h3><p style="margin:0;color:#526071;line-height:1.55">${esc(item.summary||item.description||'')}</p></span></a>`).join('');
