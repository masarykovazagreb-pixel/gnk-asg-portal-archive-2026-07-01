(() => {
  ...
  function notify() { window.dispatchEvent(new CustomEvent('gnk-live-market-refresh', {detail:{ok: live, updated_at: dataset.updated_at || null}})); }
  function convert() {
  ...
  function render() {
    ...
    if (updated && dataset.updated_at) updated.textContent = (en ? 'Updated: ' : 'Ažurirano: ') + new Date(dataset.updated_at).toLocaleString(en ? 'en-GB' : 'hr-HR') + (live ? (en ? ' · live source' : ' · izvor uživo') : (en ? ' · latest published snapshot' : ' · zadnji objavljeni presjek'));
    const ticker = $('#ticker');
    if (ticker) {
      ...
      ticker.innerHTML = tape + '<span><b>GNK ASG</b> DIGITAL ASSETS MONITOR · ' + (live ? (en ? 'LIVE APP REFRESH' : 'OSVJEŽAVANJE UŽIVO') : (en ? 'PUBLISHED FALLBACK DATA' : 'OBJAVLJENI REZERVNI PODATCI')) + '</span>' + tape;
    }
    convert();
  }
  ...
  async function directLive() {
    ...
    try {
      ...
      const response = await fetch('https://api.coingecko.com/api/v3/simple/price?' + params.toString(), {cache:'no-store', signal:controller.signal});
