...
const text=en?{idle:'Not sent.',sending:'Sending…',stored:'Your message was recorded.',sent:'Your message was recorded and the confirmation was sent.',partial:'Your message was recorded. Email delivery is pending.',filtered:'Your submission was received.',failed:'Submission failed: ',required:'Please complete all required fields and accept the privacy notice.',timeout:'The contact service did not respond in time. Please try again.',offline:'The contact service is currently unreachable. Please check your connection and try again.',rate:'Too many attempts. Please wait before trying again.'}:{idle:'Nije poslano.',sending:'Slanje…',stored:'Upit je evidentiran.',sent:'Upit je evidentiran i potvrda je poslana.',partial:'Upit je evidentiran. Dostava e-pošte je u obradi.',filtered:'Vaš je zahtjev zaprimljen.',failed:'Slanje nije uspjelo: ',required:'Ispunite sva obvezna polja i prihvatite obavijest o privatnosti.',timeout:'Kontakt usluga nije odgovorila na vrijeme. Pokušajte ponovno.',offline:'Kontakt usluga trenutačno nije dostupna. Provjerite vezu i pokušajte ponovno.',rate:'Previše pokušaja. Pričekajte prije ponovnog slanja.'};
...
form.addEventListener('submit',async event=>{
 ...
 try{
  response=await fetch('/api/contact-submit',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},credentials:'same-origin',cache:'no-store',signal:controller.signal,body:JSON.stringify(data)});
  ...
  if(out.spamFiltered){set(text.filtered,'success');form.reset();return}
  ...
  if(out.accepted&&out.stored){form.reset();form.dataset.lastCaseId=ref}
