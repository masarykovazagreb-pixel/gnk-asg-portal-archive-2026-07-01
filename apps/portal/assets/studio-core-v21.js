(()=>{
  ...
  const VERSION='GNK_ASG_STUDIO_CORE_V21_20260629_R1';
  ...
  function messageHtml(){
    return `<div style="font-family:Arial,Helvetica,sans-serif;color:#111827;font-size:15px;line-height:1.5">${esc(value('bodyText')).replace(/\r?\n/g,'<br>')}</div>`;
  }
  ...
  async function send(button){
    ...
    status('Slanje poruke kroz aktivni GNK ASG mail servis…');
    try{
      ...
      if(!response.ok||!delivered){
        ...
        status(message);
        setButtonState(button,'error','Nije poslano');
        return;
      }
      ...
      setButtonState(button,'error','Greška');
    }finally{
      button.disabled=false;
      setTimeout(()=>setButtonState(button,''),1800);
    }
  }

  function save(){
    try{localStorage.setItem('gnk_asg_studio_draft_v21',JSON.stringify({...payload(),body:value('bodyText')}));status('✓ Draft spremljen.');}
  ...
  function clearAll(){
    ['to','cc','subject','bodyText'].forEach(id=>setValue(id,''));setValue('bcc',MANDATORY_BCC);refreshPreview();status('✓ Polja su očišćena.');
  }
  ...
  async function loadBox(type){
    ...
    try{
      ...
      status(response.ok?`✓ ${type} učitan.`:`Greška učitavanja ${type}: ${response.status}`);
    }catch(error){if(list)list.textContent=error.message;status(`Greška učitavanja ${type}.`);}
  }

  const handlers={send,save,load,helper,autoReply,clear:clearAll};

  function candidates(action){
    return [...document.querySelectorAll(`#${action},#gnkMailV18_${action},#gnkMailV20_${action},#gnkStudioV21_${action},[data-gnk-v18-action="${action}"],[data-studio-action="${action}"]`)];
  }

  function installAction(action){
    const old=candidates(action)[0];
    if(!old||old.dataset.studioCoreV21==='1')return;
    const next=old.cloneNode(true);
    next.id=`gnkStudioV21_${action}`;
    next.type='button';
    next.disabled=false;
    next.dataset.studioCoreV21='1';
    ...
    next.addEventListener('pointerdown',()=>next.classList.add('studio-core-pressed'));
    const release=()=>setTimeout(()=>next.classList.remove('studio-core-pressed'),100);
    next.addEventListener('pointerup',release);next.addEventListener('pointercancel',release);next.addEventListener('pointerleave',release);
  ...
  function installTabs(){
    document.querySelectorAll('.tab[data-box]').forEach(old=>{
      if(old.dataset.studioCoreV21==='1')return;
      const next=old.cloneNode(true);
      next.type='button';next.dataset.studioCoreV21='1';next.style.pointerEvents='auto';next.style.position='relative';next.style.zIndex='3';
      old.replaceWith(next);
      next.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();loadBox(next.dataset.box);});
    });
  }
  ...
  function install(){
    ...
    status('Mail Studio je spreman za stvarno slanje.');
  }

  const style=document.createElement('style');
  style.textContent=`
    [data-studio-core-v21="1"]{cursor:pointer!important;pointer-events:auto!important;transition:transform .1s ease,filter .16s ease,box-shadow .16s ease!important;box-shadow:0 5px 0 rgba(88,57,8,.75),0 10px 24px rgba(0,0,0,.28)!important}
    [data-studio-core-v21="1"]:hover{filter:brightness(1.12)}
    [data-studio-core-v21="1"]:active,[data-studio-core-v21="1"].studio-core-pressed{transform:translateY(5px) scale(.985)!important;box-shadow:0 1px 0 rgba(88,57,8,.72),0 4px 10px rgba(0,0,0,.24)!important}
    [data-studio-core-v21="1"].studio-core-busy{cursor:progress!important;opacity:.84}
    [data-studio-core-v21="1"].studio-core-ok{border-color:#61e294!important;box-shadow:0 0 24px rgba(97,226,148,.35)!important}
    [data-studio-core-v21="1"].studio-core-error{border-color:#ff8585!important;box-shadow:0 0 22px rgba(255,133,133,.3)!important}
  `;
