(() => {
  ...
  const state = {
    ...
    invalid: [],
    pdf: null,
    currentCampaignId: localStorage.getItem('GNK_ASG_MEDIA_CAMPAIGN_ID') || ''
  };
  ...
  function parseRecipients(text) {
    ...
    lines.slice(1, 101).forEach((line, index) => {
      ...
      headers.forEach((key, position) => {
        if (key) recipient[key] = clean(values[position]);
      });
  ...
  function renderRecipients() {
    ...
    $('recipientTable').innerHTML = state.recipients.length
      ? state.recipients.slice(0, 100).map((item, index) => `
          <tr>
            <td>${index + 1}</td>
            <td>${esc(item.firstName)}</td>
            <td>${esc(item.lastName)}</td>
            <td>${esc(item.media)}</td>
  ...
  function personalise(template, recipient) {
    ...
    return String(template || '').replace(/{{\s*(firstName|lastName|fullName|media|email)\s*}}/gi, (_, key) => {
      const normalised = Object.keys(values).find(item => item.toLowerCase() === key.toLowerCase());
  ...
  async function createPreview() {
    try {
      ...
      localStorage.setItem('GNK_ASG_MEDIA_CAMPAIGN_ID', state.currentCampaignId);
      renderCampaignStatus(result.data?.batch || result.data);
      status('Kampanja je spremljena u sigurnom preview/test režimu.');
      await loadCampaigns();
    } catch (error) {
  ...
  async function campaignAction(action) {
    try {
      ...
      status(action === 'execute-window'
        ? 'Obrađen je jedan testni prozor. Stvarno masovno slanje je isključeno.'
  ...
  async function loadCampaigns() {
    ...
    $('campaignList').innerHTML = campaigns.length
      ? campaigns.map(item => `
          <button type="button" class="campaign-item" data-campaign-id="${esc(item.id)}">
            <strong>${esc(item.id)}</strong>
            <span>${esc(item.status)} · Sent ${Number(item.sent || 0)} · Tested ${Number(item.tested || 0)} · Failed ${Number(item.failed || 0)} · Remaining ${Number(item.remaining || 0)}</span>
          </button>`).join('')
