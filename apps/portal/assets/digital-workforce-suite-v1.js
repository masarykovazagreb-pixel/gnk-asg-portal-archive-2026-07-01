(()=>{
  ...
  const views={
    ...
    newsroom:d=>cards((d.items||[]).slice(0,18),x=>`<article class="dw-card"><span class="dw-kicker">${date(x.publishedAt)}</span><h3>${esc(x.title)}</h3><p>${esc(x.excerpt)}</p><small>${esc(T.editor)}: ${esc(x.editor)}</small></article>`),
