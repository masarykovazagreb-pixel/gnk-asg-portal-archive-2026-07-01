...
function trackingRecipient(item){const list=Array.isArray(item.to)?item.to:[];const first=list[0];return first?.email||first||'';}
function renderMessages(items=[],message=''){const node=$('messageList');if(!node)return;if(!items.length){node.innerHTML=`<div class="message"><b>No records</b><p>${esc(message||'No messages matched this folder/search in the current runtime.')}</p></div>`;return;}const sentFolder=state.folder==='sent';node.innerHTML=items.map(item=>`<article class="message ${state.selected===item.id?'active':''}" data-id="${esc(item.id)}" data-kind="${esc(item.direction||'MESSAGE')}"><div class="meta"><span>${esc(item.folder||item.direction||'mail')}</span>${itemStatus(item)}</div><b>${esc(item.subject||'(no subject)')}</b><div class="meta"><span>${esc(item.fromName||item.fromEmail||recipients(item.to)||'unknown')}</span><time>${esc(displayTime(item))}</time></div><p>${esc(item.textPreview||item.textBody||'No preview available.')}</p>${sentFolder?`<div class="track-summary" data-track-for="${esc(trackingRecipient(item))}" data-track-subject="${esc(item.subject||'')}">Praćenje…</div>`:''}</article>`).join('');if(sentFolder)enrichSentTracking();}
async function enrichSentTracking(){const nodes=[...document.querySelectorAll('[data-track-for]')].slice(0,40);for(const node of nodes){const recipient=node.dataset.trackFor;if(!recipient){node.textContent='';continue}try{const data=await jsonFetch(`/api/email-status/records?search=${encodeURIComponent(recipient)}&limit=3`);const subject=(node.dataset.trackSubject||'').toLowerCase();const match=(data.items||[]).find(x=>String(x.subject||'').toLowerCase()===subject)||(data.items||[])[0];if(!match){node.textContent='Nema podataka o praćenju.';continue}const opened=Number(match.open_count||0)>0;const ip=match.last_open_ip||match.last_click_ip||'';const ua=String(match.last_open_user_agent||match.last_click_user_agent||'');const device=/iPhone/.test(ua)?'iPhone':/iPad/.test(ua)?'iPad':/Android/.test(ua)?'Android':/Macintosh/.test(ua)?'Mac':/Windows/.test(ua)?'Windows':ua?'Računalo':'';const browser=/Edg\//.test(ua)?'Edge':/Chrome\//.test(ua)?'Chrome':/Firefox\//.test(ua)?'Firefox':/Safari\//.test(ua)?'Safari':'';node.innerHTML=opened?`<span class="pill">Otvoreno${Number(match.open_count)>1?` ×${match.open_count}`:''}</span> ${ip?`<code>${esc(ip)}</code>`:''} ${device?esc(device):''} ${browser?esc(browser):''}`:'<span class="pill warn">Još nije otvoreno</span>';}catch{node.textContent='';}}}
