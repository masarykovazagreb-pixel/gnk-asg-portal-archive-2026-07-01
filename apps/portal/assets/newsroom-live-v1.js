(()=>{
'use strict';
if(window.__GNK_NEWSROOM_LIVE_V2__)return;window.__GNK_NEWSROOM_LIVE_V2__=true;
...
async function render(){
  ...
  try{
    const response=await fetch(`${endpoint}&_=${Date.now()}`,{cache:'no-store',headers:{accept:'application/json'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);
    const data=await response.json();const posts=Array.isArray(data.posts)?data.posts:Array.isArray(data.items)?data.items:Array.isArray(data.news)?data.news:[];
    ...
    grid.innerHTML=posts.map(post=>{const href=post.url||post.href||(post.slug?`${english?'/en':''}/newsroom/index.html#${encodeURIComponent(post.slug)}`:`${english?'/en':''}/newsroom/index.html`);return `<article><span class="meta">${esc(post.category||post.sourceName||'News')}</span><h3>${esc(post.title||'News')}</h3><p>${esc(post.summary||post.description||'')}</p><p><small>${esc(date(post.publishedAt||post.createdAt))}</small></p><a href="${esc(href)}">${english?'Read more':'Otvori vijest'} →</a></article>`;}).join('');
