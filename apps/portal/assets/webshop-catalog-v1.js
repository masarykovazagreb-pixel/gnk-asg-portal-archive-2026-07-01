(() => {
  ...
  function render(grid) {
    ...
    grid.innerHTML = products.map(product => {
      ...
      return `<article class="shop-card" data-product-id="${esc(product.id || product.sku || '')}">${product.image ? `<div class="shop-card-image"><span class="shop-gnk-badge">GNK ASG</span><img src="${esc(product.image)}" alt="GNK ASG · ${esc(product.name)}" loading="lazy" width="480" height="300"></div>` : ''}<div class="shop-card-body"><span class="shop-gnk-label">GNK ASG · TECHNOLOGY MARKET</span><div class="shop-card-meta"><span class="shop-kicker">GNK ASG · ${esc(product.category)}</span>${availability ? `<span class="shop-availability">${esc(availability)}</span>` : ''}</div><h3>${esc(product.name)}</h3>${product.brand ? `<p class="shop-brand">GNK ASG · ${esc(product.brand)}${product.sku ? ` · ${esc(product.sku)}` : ''}</p>` : ''}<p>${esc(product.description)}</p>${tags.length ? `<div class="shop-tags">${tags.map(tag => `<span>GNK ASG · ${esc(tag)}</span>`).join('')}</div>` : ''}<div class="shop-card-footer"><div><p class="shop-price">${esc(priceLabel(product))}</p>${referenceNote}</div><button type="button" class="shop-quote-btn" data-product="${esc(product.name)}" data-product-id="${esc(product.id || product.sku || '')}">${T.requestQuote}</button></div></div></article>`;
    }).join('');
  }
