(() => {
  'use strict';
  if (window.__GNK_WEBSHOP_V5__) return;
  window.__GNK_WEBSHOP_V5__ = true;
  ...
  function render(grid) {
    const products = filteredProducts(); const count = document.getElementById('shopResultCount'); if (count) count.textContent = T.results(products.length);
    ...
    grid.innerHTML = products.map(product => {
      ...
      return `<article class="shop-card" data-product-id="${esc(product.id || product.sku || '')}">${product.image ? `<div class="shop-card-image"><img src="${esc(product.image)}" alt="${esc(product.name)}" loading="lazy" width="480" height="300"></div>` : ''}<div class="shop-card-body"><div class="shop-card-meta"><span class="shop-kicker">${esc(product.category)}</span>${availability ? `<span class="shop-availability">${esc(availability)}</span>` : ''}</div><h3>${esc(product.name)}</h3>${product.brand ? `<p class="shop-brand">${esc(product.brand)}${product.sku ? ` · ${esc(product.sku)}` : ''}</p>` : ''}<p>${esc(product.description)}</p>${tags.length ? `<div class="shop-tags">${tags.map(tag => `<span>${esc(tag)}</span>`).join('')}</div>` : ''}<div class="shop-card-footer"><div><p class="shop-price">${esc(priceLabel(product))}</p>${referenceNote}</div><button type="button" class="shop-quote-btn" data-product="${esc(product.name)}" data-product-id="${esc(product.id || product.sku || '')}">${T.requestQuote}</button></div></div></article>`;
    }).join('');
  }
  function populateCategories(select) { const categories = [...new Set(state.products.map(product => product.category).filter(Boolean))].sort((a, b) => a.localeCompare(b, en ? 'en' : 'hr')); select.innerHTML = `<option value="">${T.all}</option>${categories.map(category => `<option value="${esc(category)}">${esc(category)}</option>`).join('')}`; }
  ...
  function wireQuoteButtons(grid, form) {
    grid.addEventListener('click', event => { const button = event.target.closest('.shop-quote-btn'); if (!button || !form) return; const name = button.dataset.product, id = button.dataset.productId, subject = form.elements.subject, message = form.elements.message, department = form.elements.department; if (subject) subject.value = `${T.subjectPrefix} ${name}`; if (message) message.value = `${T.messageTemplate(name)}${id ? `Referenca sadržaja: ${id}\n` : ''}`; if (department && [...department.options].some(option => option.value === 'webshop')) department.value = 'webshop'; const note = document.getElementById('shopSelectedNote'); if (note) note.textContent = `${T.selected} ${name}${id ? ` (${id})` : ''}`; form.scrollIntoView?.({ behavior: 'smooth', block: 'start' }); if (message) { message.focus(); message.setSelectionRange(message.value.length, message.value.length); } });
  }
  function renderCatalogMeta(data, bestBuyMeta, dummyCount) { const meta = document.getElementById('shopCatalogMeta'); if (!meta) return; const parts = []; if (data.updatedAt) { const date = new Date(data.updatedAt); parts.push(T.updated(Number.isNaN(date.getTime()) ? data.updatedAt : new Intl.DateTimeFormat(en ? 'en-GB' : 'hr-HR', { dateStyle: 'medium', timeStyle: 'short' }).format(date))); } if (data.sourceLabel) parts.push(T.source(data.sourceLabel)); if (bestBuyMeta?.fx?.rate) parts.push(`ECB EUR/USD ${bestBuyMeta.fx.rate}${bestBuyMeta.fx.date ? ` · ${bestBuyMeta.fx.date}` : ''}`); if (dummyCount) parts.push(`DummyJSON ${dummyCount}`); meta.textContent = parts.join(' · '); }
