...
function kpis(summary={}){
 const defs=[['DRAFT','Nacrti'],['SUBMITTED','Predano'],['NEEDS_INFORMATION','Dopuna'],['APPROVED','Odobreno'],['NEEDS_TRAVEL_DOCUMENTS','Putni dokumenti'],['TRAVEL_CONFIRMED','Put potvrđen']];
 document.getElementById('mediaReviewKpis').innerHTML=defs.map(([key,name])=>`<article class="media-review-kpi"><b>${Number(summary[key]||0).toLocaleString('hr-HR')}</b><span>${esc(name)}</span></article>`).join('');
}
...
async function refresh(){
 ...
 catch(error){document.getElementById('mediaReviewBody').innerHTML=`<tr><td colspan="9" class="media-review-empty">${esc(error.message)}</td></tr>`;}
...
function csv(){
 ...
 const headers=['Application ID','Mail Code','Newsroom','Brand','Country','Email','Editor','Participants','Documents','Completion','Status','Updated'];
 const rows=state.items.map(item=>[item.applicationId,item.mailCode,item.outlet,item.brandName,item.country,item.email,item.editorName,item.participants,item.documentCount,item.completionScore,item.status,item.updatedAt]);
