(() => {
  ...
  const modules=[
    ...
    {id:'operator',label:'Operator',icon:'▣',title:'Operator centar',description:'Status, konfiguracija, inbox, potpisi, evidencije i operativne postavke.',route:'/operator-dashboard/'},
    {id:'mail',label:'Mail Studio',icon:'✉',title:'Mail Studio',description:'Poruke, potpisi, PDF prilozi, nacrti i AI pomoć.',route:'/mail-studio/'},
  ...
  function embedded(route){const url=new URL(route,location.origin);url.searchParams.set('embedded','1');return url.pathname+url.search;}
  function select(id,push=true){
    ...
    q('gnkHubTitle').textContent=item.title;q('gnkHubDescription').textContent=item.description;q('gnkHubStandalone').href=item.route;
    if(item.id==='overview'){
      ...
      const target=embedded(item.route);loading.hidden=false;frame.hidden=true;
      if(frame.dataset.route!==target){frame.dataset.route=target;frame.src=target;}else{loading.hidden=true;frame.hidden=false;}
    }
  ...
  frame.addEventListener('load',()=>{loading.hidden=true;frame.hidden=false;});
