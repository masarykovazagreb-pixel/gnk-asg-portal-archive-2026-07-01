(() => {
  ...
  function renderKpis(){
    const status=state.status||{},contacts=status.contacts||{},applications=status.applications||{},readiness=status.readiness||{};
    const items=[['Kontakti',contacts.total||0],['Operativno spremni',readiness.ready||0],['Čekaju odobrenje',readiness.awaitingApproval||0],['Za provjeru',readiness.needsVerification||0],['Blokirani',readiness.blocked||0],['Prijave',applications.total||0],['Za ljudsku provjeru',applications.ready||0],['Odobrene prijave',applications.approved||0]];
    $('kpis').innerHTML=items.map(([label,value])=>`<article class="mcc-kpi"><span>${esc(label)}</span><strong>${esc(value)}</strong></article>`).join('');
    const issues=(readiness.needsVerification||0)+(readiness.blocked||0)+(applications.incomplete||0);
    const attention=$('attentionQueue');
    if(issues){attention.hidden=false;attention.innerHTML=`<strong>Zahtijeva pažnju:</strong> ${esc(readiness.needsVerification||0)} kontakata za provjeru, ${esc(readiness.blocked||0)} blokiranih kontakata i ${esc(applications.incomplete||0)} nepotpunih prijava.`;}else attention.hidden=true;
  }

  function renderSystem(){
    const status=state.status||{},bindings=status.bindings||{},rate=status.rate||{},readiness=status.readiness||{};
    $('environmentBadge').textContent=status.live?'LIVE':'DRY-RUN · SLANJE ZAKLJUČANO';
    ...
    $('systemCards').innerHTML=[
      ...
      ['Kontakti READY',readiness.ready||0],['Rok prijave','20. 7. 2026.'],['Konačna odluka','LJUDSKA']
  ...
  function openApplicationDetail(id){const application=state.applications.find(item=>item.applicationId===id);if(!application)return;$('applicationDetail').innerHTML=`<h3>${esc(application.applicationId)}</h3><pre>${esc(JSON.stringify(application,null,2))}</pre>`;}
