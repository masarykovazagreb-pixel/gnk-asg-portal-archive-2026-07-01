(()=>{
  ...
  function deliveryCell(access){
    if(!access)return '<span class="admin-media-pill warn">NEPOZNATO</span><br><small>Kontakt nije pronađen u delivery registru.</small>';
    const sent=access.sent_status||'NIJE POSLANO';
    const response=access.response_status||'NEMA ODGOVORA';
    const sentClass=/POSLANO|SENT|DELIVERED/i.test(sent)?'ok':/FAILED|BOUNCE|ODBIJ/i.test(sent)?'bad':'warn';
    const responseClass=/ODGOVOR|REPLIED|RECEIVED|ZAPRIM/i.test(response)&&!/NEMA/i.test(response)?'ok':/ODBIJ|DECLIN/i.test(response)?'bad':'warn';
    return '<span class="admin-media-pill '+sentClass+'">'+esc(sent)+'</span><br><span class="admin-media-pill '+responseClass+'">'+esc(response)+'</span><br><small>Odobreno: '+esc(access.approved?'DA':'NE')+' · automatizacija: '+esc(access.automation_allowed?'DA':'NE')+'</small>';
  }
