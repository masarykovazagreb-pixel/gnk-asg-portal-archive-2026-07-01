(()=>{
  ...
  function accessMap(items){
    const map=new Map();
    for(const item of items||[])map.set(String(item.mail_code||'').toUpperCase(),item);
    return map;
  }
  function accessCell(item,access){
    ...
    return '<span class="admin-media-pill '+cls+'">'+esc(status)+'</span><br><small>Poslano '+esc(access.access_sent_at||'—')+'</small><br><small>Vrijedi do '+esc(access.access_expires_at||'—')+'</small><br><small>Pokušaji '+esc(access.access_attempts??0)+'</small>';
  }
