...
const boot=()=>{
 ...
 groups.forEach(group=>{const section=document.createElement('section');section.className='section';const label=document.createElement('span');label.className='label'+(group.protected?' locked':' open');label.textContent=(group.protected?'\uD83D\uDD12 ':'')+group.label;const links=document.createElement('div');links.className='links';group.items.forEach(([hrLabel,enLabel,hrHref,enHref])=>{const a=document.createElement('a');a.className=group.protected?'locked':'open';a.href=route(hrHref,enHref);const target=new URL(a.href,location.origin).pathname.replace(/\/+$/,'')||'/';if(target===path)a.setAttribute('aria-current','page');const text=document.createElement('span');text.textContent=english?enLabel:hrLabel;a.appendChild(text);if(group.protected){a.rel='nofollow';const badge=document.createElement('span');badge.className='protected';badge.textContent=english?'PROTECTED':'ZAŠTIĆENO';a.appendChild(badge)}links.appendChild(a)});section.append(label,links);body.appendChild(section)});
