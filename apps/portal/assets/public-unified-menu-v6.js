...
const boot=()=>{
 ...
 const groups=[
  {label:english?'PUBLIC PORTAL':'JAVNI PORTAL',items:[
   ['Početna','Home','/','/en/'],['O nama','About','/about/','/en/about/'],['Nermin Sefić','Nermin Sefić','/nermin-sefic/','/en/nermin-sefic/'],['Grupna mreža','Group network','/group-network/','/en/group-network/'],['Payment Infrastructure (BPP) — GNK DINAMO Ltd.','Payment Infrastructure (BPP) — GNK DINAMO Ltd.','https://bpp.is/','https://bpp.is/'],['Financije','Finance','/financije/','/en/finance/'],['Newsroom','Newsroom','/newsroom/','/en/newsroom/'],['AKTUAL MEDIA','AKTUAL MEDIA','/gnk-aktual/','/en/gnk-aktual/'],['Objave','Publications','/objave/','/en/publications/'],['Analize','Analyses','/analize/','/en/analyses/'],['Komentari','Commentary','/komentari/','/en/commentary/'],['Tržišta','Markets','/trzista/','/en/markets/'],['Puls Tržišta','Puls Tržišta','/puls-trzista/','/puls-trzista/'],['Tržišne krize','Market Crises','/trzisne-krize/','/en/market-crises/'],['Projekti','Projects','/projects/','/en/projects/'],['Nilus Bio (PRJ-006)','Nilus Bio (PRJ-006)','/nilus-bio/','/nilus-bio/'],['Izvješća','Reports','/reports/','/en/reports/'],['Knowledge Center','Knowledge Center','/knowledge-center/','/en/knowledge-center/'],['Tehnologija','Technology','/tehnologija/','/en/technology/'],['GNK DINAMO Digital Solutions','GNK DINAMO Digital Solutions','/digital-solutions/','/digital-solutions/'],['Pulse — Command Center Demo','Pulse — Command Center Demo','/pulse-demo/','/pulse-demo/'],['Digitalna radna snaga','Digital workforce','/digital-workforce/','/digital-workforce/'],['SYNAPSE — AI Agent Simulation','SYNAPSE — AI Agent Simulation','/synapse-demo/','/synapse-demo/'],['THE CODE','THE CODE','/the-code/','/en/the-code/'],['Medijske prijave','Media applications','/media-application/','/media-application/?lang=en'],['Vizualni indeks','Visual Index','/visual-index/','/en/visual-index/'],['Workeri','Workers','/workers/','/en/workers/'],['Registri','Registries','/registri/','/en/registries/'],['Intelligence Desk','Intelligence Desk','/intelligence-desk/','/en/intelligence-desk/'],['Instalacija','Install','/instalacija/','/en/instalacija/'],['Sadržaj portala','Site content','/sadrzaj/','/en/sadrzaj/'],['Tematski monitoring','Thematic monitoring','/teme/','/en/teme/'],['Autorske objave','Author publications','/autorske-objave/','/en/autorske-objave/'],['Insights arhiva','Insights archive','/insights-hr/','/insights-hr/'],['Kontakt','Contact','/contact/','/en/contact/'],['Trgovina','Shop','/trgovina/','/trgovina/'],['Pravne napomene','Legal Notices','/legal/','/en/legal/'],['Ideje u djelovanju','Ideje u djelovanju','/ideje-u-djelovanju/','/ideje-u-djelovanju/']
  ]},
  {label:'ADMIN CENTER',protected:true,items:[
 ...
 if(existing&&existing.dataset.gnkUnifiedShell==='v6-static'){
  ...
  if(!document.getElementById('gnk-unified-menu-css')){
   const link=document.createElement('link');link.id='gnk-unified-menu-css';link.rel='stylesheet';link.href='/assets/public-unified-menu-v6.css?v=20260721-static-header';
 ...
 const head=document.createElement('div');head.className='nav-head';head.innerHTML=`<span class="nav-title">${english?'Navigation':'Navigacija'}</span><span class="nav-note">GNK ASG</span>`;const body=document.createElement('div');body.className='nav-body';
 groups.forEach(group=>{const section=document.createElement('section');section.className='section';const label=document.createElement('span');label.className='label'+(group.protected?' locked':' open');label.textContent=(group.protected?'\uD83D\uDD12 ':'')+group.label;const links=document.createElement('div');links.className='links';group.items.forEach(([hrLabel,enLabel,hrHref,enHref])=>{const a=document.createElement('a');a.className=group.protected?'locked':'open';a.href=route(hrHref,enHref);const isExternal=/^https?:\/\//i.test(a.href);if(isExternal){a.target='_blank';a.rel=(a.rel?a.rel+' ':'')+'noopener'}const target=isExternal?'':new URL(a.href,location.origin).pathname.replace(/\/+$/,'')||'/';if(target&&target===path)a.setAttribute('aria-current','page');const text=document.createElement('span');text.textContent=english?enLabel:hrLabel;a.appendChild(text);if(group.protected){a.rel=(a.rel?a.rel+' ':'')+'nofollow';const badge=document.createElement('span');badge.className='protected';badge.textContent=english?'PROTECTED':'ZAŠTIĆENO';a.appendChild(badge)}links.appendChild(a)});section.append(label,links);body.appendChild(section)});
 ...
 const close=()=>{menu.classList.remove('open');button.setAttribute('aria-expanded','false');button.textContent=english?'MENU':'IZBORNIK'};button.addEventListener('click',()=>{const open=menu.classList.toggle('open');button.setAttribute('aria-expanded',String(open));button.textContent=open?(english?'CLOSE':'ZATVORI'):(english?'MENU':'IZBORNIK')});document.addEventListener('click',event=>{if(menu.classList.contains('open')&&!menu.contains(event.target))close()});document.addEventListener('keydown',event=>{if(event.key==='Escape')close()});
};
