(()=>{
  'use strict';
  const RELEASE='GNK_ASG_MAIL_STUDIO_INBOX_UI_V2_CENTERS_20260628';
  ...
  function render(payload){
    ...
    list.innerHTML=`<div class="gnk-inbox-summary"><strong>Inbox · ${items.length}</strong><small>10 digitalnih centara zaprimanja</small></div>`+(items.length?`<div class="gnk-inbox-items">${items.map(item=>{
      const from=item.from||{},attachment=item.attachment||{},center=item.receivingCenter||{};
      const centerLabel=center.labelHr||[center.cityHr,center.countryHr,center.code].filter(Boolean).join(' · ');
      return `<article class="gnk-inbox-card"><div class="gnk-inbox-card__top"><div><strong>${esc(item.subject||'(bez predmeta)')}</strong><br><small>${esc(from.name||'')} · ${esc(from.email||'')}</small></div><small>${esc(formatDate(item.receivedAt))}</small></div><p>${esc(item.snippet||item.message||'')}</p>${centerLabel?`<div class="gnk-inbox-center">Zaprimljeno putem: ${esc(centerLabel)}</div>`:''}<div class="gnk-inbox-meta"><span>${esc(item.caseId||'bez evidencijskog broja')}</span><span>${esc(item.mailboxLabel||item.mailboxKey||'odjel')}</span><span>${esc(item.status||'received')}</span>${attachment.filename?`<span>PDF · ${esc(attachment.filename)}</span>`:''}</div><button type="button" class="gnk-inbox-reply" data-inbox-reply="${esc(item.caseId||'')}" data-email="${esc(from.email||'')}" data-name="${esc(from.name||'')}" data-subject="${esc(item.subject||'')}" data-message="${esc(item.message||'')}" data-mailbox="${esc(item.mailboxKey||'')}" data-center="${esc(centerLabel)}">Pripremi odgovor</button></article>`;
  ...
  function prepareReply(button){
    const email=button.dataset.email||'',name=button.dataset.name||'',subject=button.dataset.subject||'',message=button.dataset.message||'',mailbox=button.dataset.mailbox||'',center=button.dataset.center||'';
    ...
    if($('aiContext'))$('aiContext').value=[message,center?`Digitalni centar zaprimanja: ${center}`:''].filter(Boolean).join('\n\n');
