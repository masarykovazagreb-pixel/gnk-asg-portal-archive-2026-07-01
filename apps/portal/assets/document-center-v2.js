(() => {
  ...
  function buildCards() {
    ...
    const cards = isEn ? [
      ...
      [dinamoReport,'▣','GNK DINAMO Ltd. FY 2025','Management-certified and internally reviewed consolidated report; unaudited.'],
      [certificate,'✓','Certificate of Good Standing','Official Colorado Secretary of State certificate, issued 16 June 2026.'],
      ...
      [dinamoReport,'▣','GNK DINAMO Ltd. FY 2025','Upravljački potvrđeno i interno pregledano konsolidirano izvješće; nerevidirano.'],
      [certificate,'✓','Certificate of Good Standing','Službeni certifikat Colorado Secretary of State, izdan 16. lipnja 2026.'],
      [hrKit,'▤','Media Kit 2026 — Hrvatski','Novi sedmostranični hrvatski korporativni i medijski paket.'],
      [enKit,'▤','Corporate Media Kit 2026','New seven-page English corporate and media package.'],
      [centre,'⇩','PDF i Download centar','Svi izvorni dokumenti, Media Kitovi i napomene o provjeri.']
    ];
    grid.innerHTML = cards.map(([href,icon,title,description],index) => `<a class="pdf-card" href="${href}"${index<5?' target="_blank" rel="noopener"':''}><span>${icon}</span><b>${title}</b><small>${description}</small></a>`).join('');
  }
