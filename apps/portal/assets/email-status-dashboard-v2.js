...
if(dateFilter==='today')window.fetch=(input,init)=>{
 try{
  if(typeof input==='string'||input instanceof URL){const url=new URL(String(input),location.origin);if(url.origin===location.origin&&url.pathname==='/api/email-status/records'){url.searchParams.set('date','today');input=url.pathname+url.search;}}
 }catch{}
...
const boot=()=>{
 ...
 if(top&&!document.querySelector('.gnk-status-help')){
  ...
  const dateToggle=document.createElement('a');dateToggle.className='gnk-status-action';const dateUrl=new URL(location.href);if(dateFilter==='today'){dateUrl.searchParams.delete('date');dateToggle.textContent='Prikaži sve datume';}else{dateUrl.searchParams.set('date','today');dateToggle.textContent='Samo današnje poruke';}dateToggle.href=dateUrl.pathname+dateUrl.search;actions.append(dateToggle);
  ...
  const note=document.createElement('div');note.className='gnk-status-note';if(dateFilter==='today')note.textContent='Prikazuju se poruke od 00:00 do 24:00 prema vremenskoj zoni Europe/Zagreb.';
  sync.onclick=async()=>{sync.disabled=true;note.textContent='Usklađivanje je u tijeku…';try{const response=await nativeFetch('/api/email-status/sync',{method:'POST',credentials:'same-origin',cache:'no-store'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||data.error||`HTTP ${response.status}`);note.textContent=`Usklađivanje završeno. Obrađeno događaja: ${data.events??data.eventCount??data.queried??0}.`;load?.click();}catch(error){note.textContent=`Usklađivanje nije završeno: ${error.message}`;}finally{sync.disabled=false;}};actions.append(sync);
