...
function bind(){
  renderBadge();
  const send=$('send');
  if(send&&!send.dataset.referenceGuard){
    send.dataset.referenceGuard='1';
    send.addEventListener('click',event=>{event.stopImmediatePropagation();event.preventDefault();enforceReference().then(()=>{send.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window,detail:1}));}).catch(error=>status(`Reference code failed: ${error.message}`));},{capture:true,once:true});
  }
}
