...
const result = await page.evaluate(() => {
  ...
  return {
    bottomShowcase: Boolean(document.querySelector('main > #the-code-index,main > .gnk-code-slot')),
...
if (!result.visualInsideMap) failures.push('visuals are outside the existing map field');
if (!result.codeInsideLocations) failures.push('THE CODE is outside the existing locations field');
if (result.imageCount !== 2 || result.loadedImages !== 2) failures.push('two visuals are not loaded');
if (!result.iframeSource.startsWith('/the-code/')) failures.push('THE CODE source is invalid');
if (!result.shellRect || result.shellRect.height < 500) failures.push('THE CODE is too short');
if (result.visualRect && result.mapRect && result.visualRect.bottom > result.mapRect.bottom + 1) failures.push('visuals overflow their existing field');
if (result.codeRect && result.locationsRect && result.codeRect.bottom > result.locationsRect.bottom + 1) failures.push('THE CODE overflows its existing field');
