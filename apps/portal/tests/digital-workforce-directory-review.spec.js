...
test('Digital Workforce directory renders 1,537 generated profiles in review mode', async ({ page }) => {
  ...
  await expect(page.locator('html')).toHaveAttribute('data-review-languages', 'hr,en');
  ...
  await expect(page.locator('body')).toHaveAttribute('data-review-language-contract', 'hr-en');
  await expect(page.locator('body')).toHaveAttribute('data-review-hr-en-parity', 'true');
  await expect(page.locator('[data-review-disclosure]')).toContainText('system-generated operational profiles');
  await expect(page.locator('[data-review-multilingual-contract]')).toBeVisible();
  await expect(page.locator('[data-review-multilingual-contract] [lang="hr"]')).toContainText('HR ugovor pregleda');
  await expect(page.locator('[data-review-multilingual-contract] [lang="en"]')).toContainText('EN review contract');
