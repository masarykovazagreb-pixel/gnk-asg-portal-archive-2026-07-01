  ...
  for (const [route, name, locale] of routes) {
    ...
    try {
      ...
      const diagnostics = await page.evaluate(() => {
        ...
        const visible = [...document.querySelectorAll(legacySelectors.join(','))].filter(element => {
        ...
        const overflowElements = overflow ? [...document.querySelectorAll('body *')].map(element => {
          const rect = element.getBoundingClientRect();
        ...
        }).filter(entry => entry.right > innerWidth + 2 || entry.left < -2).slice(0, 15) : [];
        return { visible, overflow, overflowElements };
      });
      ...
      item.overflowElements = diagnostics.overflowElements;
