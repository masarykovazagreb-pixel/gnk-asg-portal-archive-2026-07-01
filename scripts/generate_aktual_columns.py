...
def render(item: dict, lang: str) -> str:
    ...
    image_path = item.get("slika") or "/assets/gnk-asg-social-card.png"
    ...
    schema = {"@context":"https://schema.org","@type":"OpinionNewsArticle","headline":title,"description":description,"url":canonical,"mainEntityOfPage":{"@type":"WebPage","@id":canonical},"image":{"@type":"ImageObject","url":image_url},"author":{"@type":"Person","name":"Nermin Sefić"},"publisher":{"@type":"Organization","name":"GNK ASG d.o.o.","logo":{"@type":"ImageObject","url":PUBLISHER_LOGO}},"datePublished":published,"dateModified":published,"inLanguage":language,"isAccessibleForFree":True}
...
<a class="natrag" href="{back}">{back_text}</a><br><a class="alt-jezik" href="{alternate}">{alternate_text}</a><article><span class="oznaka">{label}</span><h1>{esc(title)}</h1><img src="{esc(image_path)}" alt="{esc(title)}" style="max-width:100%;border:3px solid #241C0E;margin-bottom:20px">
{paragraphs(text)}
</article></body></html>'''
