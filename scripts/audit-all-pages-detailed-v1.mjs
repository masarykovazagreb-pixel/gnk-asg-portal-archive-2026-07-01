...
const IGNORE_HTML_PREFIXES=['assets/'];
...
function formIssues(html){const issues=[];for(const form of html.matchAll(/<form\b[\s\S]*?<\/form>/gi)){const fragment=form[0];for(const control of fragment.matchAll(/<(input|select|textarea)\b([^>]*)>/gi)){const tag=control[1].toLowerCase(),attrs=control[2],type=(attrs.match(/\btype=["']?([^\s"'>]+)/i)?.[1]||'').toLowerCase();if(['hidden','submit','button','reset','checkbox','radio','file'].includes(type))continue;const id=attrs.match(/\bid=["']([^"']+)["']/i)?.[1];const aria=attrs.match(/\baria-label=["'][^"']+["']/i)||attrs.match(/\baria-labelledby=["'][^"']+["']/i);const labelled=id&&new RegExp(`<label\\b[^>]*for=["']${id.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}["']`,'i').test(fragment);if(!aria&&!labelled)issues.push({tag,id:id||null,issue:'missing-accessible-label'})}}return issues}
...
const htmlFiles=walk(ROOT).filter(file=>file.endsWith('.html')&&!IGNORE_HTML_PREFIXES.some(prefix=>rel(file).startsWith(prefix)));
...
for(const file of htmlFiles){
 ...
 const logoRefs=extract(html,'src').filter(src=>/logo|gnk[_-]?asg|the-code/i.test(src));
