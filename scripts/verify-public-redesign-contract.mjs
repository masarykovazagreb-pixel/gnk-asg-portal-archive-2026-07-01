...
if(groups.has('registration')){
  ...
  requireAll('registration','workers/gnk-asg-direct-operator/src/media-registration-v1.js',['export const PUBLIC_UI=legacy.PUBLIC_UI',"PUBLIC_API='/api/media-registration'","COOKIE='gnk_asg_media_registration'",'ensureLegacySchema','processMediaInvitationQueue','handleMediaRegistrationPublic']);
  requireAll('registration','workers/gnk-asg-direct-operator/src/media-registration-legacy-v1.js',["PUBLIC_UI='/media-application'",'HttpOnly; Secure; SameSite=Strict','media_registration_sessions','media_registration_drafts','media_registration_documents']);
}
...
if(groups.has('entrypoints')){
  ...
  requireCondition('entrypoints','review Worker must use the current Digital Workforce entrypoint',review.includes('main = "src/index-digital-workforce-v1.js"'));
  ...
  requireCondition('entrypoints','review scheduled publication must remain disabled',review.includes('NEWS_AUTO_PUBLICATION_SCHEDULED_LIVE = "false"'));
