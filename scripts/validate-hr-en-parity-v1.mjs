...
function canonical(html) {
  ...
  return normalizeHref(tag?.match(/href=["']([^"']+)["']/i)?.[1]);
}
...
for (const file of walk(root)) {
  ...
  for (const lang of ['hr', 'en', 'x-default']) {
    ...
    if (!target) continue;
    if (target.includes('/en/en/')) { errors.push(`${route}: ${lang} points to forbidden ${target}`); continue; }
    const tf = fileForRoute(target);
    if (!tf || !fs.existsSync(tf)) errors.push(`${route}: ${lang} target missing on disk: ${target}`);
  }
...
const report = { generatedAt: new Date().toISOString(), checkedPages: checked.length, errors, warnings };
