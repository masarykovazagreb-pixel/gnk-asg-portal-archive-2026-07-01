...
const fields=['name','workerId','department','role','countryRegion','languages','timezone','status','entitySlot','responsibility'];
...
check(seo.indexingPolicy.automaticPublication===false,'automatic publication must remain false');
...
check(!read('workers/gnk-asg-direct-operator/src/editorial-workflow-v1.js').includes("state:'published'"),'automatic published state forbidden');
...
console.log(JSON.stringify({profiles:profiles.length,departments:departments.size,entitySlots:entities.size,mandatoryHumanApprovals:approvals.length}));
