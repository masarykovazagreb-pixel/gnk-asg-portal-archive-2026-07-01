...
def main() -> int:
    ...
    for route, item in sorted(published.items()):
        ...
        if not page.is_file():
            ...
            continue
        parser = HeadParser(); parser.feed(page.read_text(encoding="utf-8", errors="replace"))
        expected = ORIGIN + route
        www_alias = expected.replace("https://gnk-asg.hr", "https://www.gnk-asg.hr", 1)
        if parser.canonicals == [www_alias]:
            warnings.append(f"Legacy www canonical alias should be normalized to apex: {route}")
    ...
    evidence = {
        ...
        "canonicalAuthority": ["apps/portal/data/editorial-registry.json", "apps/portal/data/editorial-registry-supplement.json"],
        "now": now.isoformat(), "registryItems": len(items), "supplementedUniqueRoutes": supplemented,
        "publishedRoutes": len(published), "scheduledOrHeldRoutes": len(scheduled), "editorialSitemapUrls": len(rows),
        "hrPublishedRoutes": sum(not route.startswith("/en/") for route in published),
        "enPublishedRoutes": sum(route.startswith("/en/") for route in published), "errors": errors, "warnings": warnings,
    }
    out = ROOT / "artifacts" / "publication-sitemap-registry"; out.mkdir(parents=True, exist_ok=True)
