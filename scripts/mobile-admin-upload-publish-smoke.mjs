...
add('Secure operator protects media upload', secureOperator.includes("'/api/media-upload'") && /status\s*:\s*401/.test(secureOperator) && secureOperator.includes('protectedCorsHeaders(request)'));
...
class FakeKV {
  ...
  async put(key, value) { this.puts.push({ key, value: String(value) }); this.values.set(key, String(value)); }
}
...
const dryRun = await call('/api/publications/publish', { method: 'POST', headers, body: JSON.stringify({ ...validPayload, dryRun: true }) });
...
const shortRun = await call('/api/publications/publish', { method: 'POST', headers, body: JSON.stringify({ ...validPayload, bodyHtml: '<p>Premalo riječi.</p>', dryRun: true }) });
...
const liveSimulation = await call('/api/publications/publish', { method: 'POST', headers, body: JSON.stringify({ ...validPayload, dryRun: false }) });
...
const duplicate = await call('/api/publications/publish', { method: 'POST', headers, body: JSON.stringify({ ...validPayload, dryRun: false }) });
...
const update = await call('/api/publications/publish', { method: 'POST', headers, body: JSON.stringify({ ...validPayload, allowUpdate: true, dryRun: false }) });
...
const report = {
  generatedAt: new Date().toISOString(), scope: 'isolated-mobile-admin-upload-publish-smoke',
  productionNetworkUsed: false, productionWrites: false, livePublishSimulation: 'fake-kv-only',
  liveBulkMailSending: false, status: failed.length ? 'FAIL' : 'PASS',
