// Jednokratni backfill: objavljuje SVE nezapočete queue stavke odmah,
// zadržavajući originalno zakazane datume/vremena kao publishedAt.
// Sav ostatak logike identičan content-queue-publish-v1.mjs (registry, sitemap).
...
const pending=queue.items.filter(x=>!state.published[x.id]);
console.log(`Backfill: total in queue=${queue.items.length}, already published=${Object.keys(state.published).length}, backfilling now=${pending.length}`);
if(!pending.length){console.log(JSON.stringify({changed:false}));process.exit(0);}
...
for(const it of pending){
  ...
  console.log('PUBLISH',it.id,'('+it.date+')',canonical);
}
...
console.log(JSON.stringify({changed:published.length>0,publishedCount:published.length},null,2));
