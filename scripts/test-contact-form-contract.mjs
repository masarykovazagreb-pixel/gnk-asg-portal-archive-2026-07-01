...
console.log(JSON.stringify({ok:true,forms:['hr','en'],endpoint:'/api/contact-submit',payloads:['json','multipart'],storage:'D1',mailTransport:'Cloudflare EmailMessage',mail:['internal','ai-or-deterministic-acknowledgement'],inlineLogo:true,rateLimit:['ip','email'],scheduledMail:{mailStudio:'v4',reviewFailClosed:true,featureFlagRequired:true,emailBindingRequired:true},campaignQueue:{reviewFailClosed:true,featureFlagRequired:true,emailBindingRequired:true},reviewMailFailClosed:true,reviewContactAiDisabled:true,productionContactAiEnabled:true,mailSent:false},null,2));
