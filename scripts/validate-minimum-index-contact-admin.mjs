...
for(const [name,html] of [['HR kontakt',contactHr],['EN kontakt',contactEn]]){
  assert.ok(html.includes('id="contactForm"'),`${name} mora zadržati kanonski kontakt obrazac.`);
  assert.ok(html.includes('name="consent"')&&html.includes('required'),`${name} mora zadržati obveznu privolu.`);
  assert.ok(html.includes('name="website"')&&html.includes('honeypot'),`${name} mora zadržati anti-bot honeypot.`);
  assert.ok(html.includes('name="email"')&&html.includes('type="email"'),`${name} mora zadržati validirani e-mail ulaz.`);
  assert.ok(html.includes('name="message"')&&html.includes('textarea'),`${name} mora zadržati sadržaj upita.`);
}
...
if(reviewMain.includes('digital-workforce')){
  ...
  assert.ok(/runtimeHealth:\s*\{\s*verified:\s*Boolean\(/.test(reviewSource),'Runtime health smije biti verified samo iz health odgovora.');
