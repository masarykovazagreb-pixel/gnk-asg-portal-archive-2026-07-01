...
ROOT=Path.cwd();SRC=ROOT/'ops'/'high-throughput-capability-extension-v14.json';OUT=ROOT/'artifacts'/'high-throughput-capability-extension-v14';errors=[]
...
report={'version':'GNK_ASG_HIGH_THROUGHPUT_CAPABILITY_EXTENSION_V14_VALIDATION','ok':not errors,'capabilityCount':len(seen),'errors':errors};OUT.mkdir(parents=True,exist_ok=True);(OUT/'report.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8');print(json.dumps(report,indent=2));raise SystemExit(1 if errors else 0)
