...
p=Path('ops/high-throughput-capability-extension-v23.json')
...
report={'version':'GNK_ASG_HIGH_THROUGHPUT_CAPABILITY_EXTENSION_V23_VALIDATION','ok':not errors,'taskClasses':len(classes),'errors':errors}
out=Path('artifacts/high-throughput-capability-extension-v23');out.mkdir(parents=True,exist_ok=True);(out/'report.json').write_text(json.dumps(report,indent=2)+'\n')
