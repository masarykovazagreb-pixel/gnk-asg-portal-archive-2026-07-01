...
ROOT=Path.cwd(); src=ROOT/'ops'/'high-throughput-capability-extension-v5.json'; out=ROOT/'artifacts'/'high-throughput-capability-extension-v5'; out.mkdir(parents=True,exist_ok=True)
errors=[]
try: data=json.loads(src.read_text())
except Exception as e: data={}; errors.append(f'cannot parse capability extension v5: {e}')
...
for i,c in enumerate(caps):
    ...
    if len(c.get('fallbackPool',[]))<2 or len(set(c.get('fallbackPool',[])))<2: errors.append(f"{c['taskClass']}: at least two distinct fallbacks required")
...
report={'version':'GNK_ASG_HIGH_THROUGHPUT_CAPABILITY_EXTENSION_V5_VALIDATION','ok':not errors,'capabilityCount':len(caps),'errors':errors}
