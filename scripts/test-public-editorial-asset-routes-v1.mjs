...
const env={ASSETS:{async fetch(request){
 ...
 assert.ok(url.pathname.endsWith('/'),`ASSETS request must use canonical trailing slash: ${url.pathname}`);
...
for(const [path,{assetPath,requestPath}] of cases){
 ...
 assert.deepEqual(seen[before],{path:requestPath,method:'GET',redirect:'follow'});
...
console.log(JSON.stringify({ok:true,version:VERSION,roots:EDITORIAL_ROOTS.length,runtimeCases:cases.size,assetFetches:seen.length,plannedPackages:plan.packages.length,canonicalAssetRequests:true,cache:{collections:'no-store',articles:'120s'}},null,2));
