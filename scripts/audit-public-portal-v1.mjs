...
const external=ref=>!ref||ref.startsWith('#')||/^(?:https?:|mailto:|tel:|data:|javascript:)/i.test(ref)||ref.startsWith('//');
const targetExists=(ref,file)=>{
  const clean=normalizeRef(ref);if(external(clean)||clean.startsWith('/api/'))return true;
  if(clean.startsWith('/')){
...
for(const file of htmlFiles){
  const html=fs.readFileSync(file,'utf8');const route=routeFor(file);const protectedRoute=isProtected(route);const refs=[...extract(html,'href'),...extract(html,'src')];
  const broken=refs.filter(ref=>!targetExists(ref,file));
  ...
  const indexable=!protectedRoute&&!/noindex/i.test(robots);
  const canon=canonical(html);
  const lang=html.match(/<html\b[^>]*lang=["']([^"']+)["']/i)?.[1]||'';
  const item={route,file:posix(path.relative('.',file)),protected:protectedRoute,indexable,lang,canonical:canon,broken,legacy,menuCount};pages.push(item);
  for(const ref of broken)findings.push({severity:'error',code:'BROKEN_LOCAL_REFERENCE',route,file:item.file,value:ref});
  for(const token of legacy)findings.push({severity:'error',code:'LEGACY_RUNTIME_REFERENCE',route,file:item.file,value:token});
  ...
  if(protectedRoute&&indexable)findings.push({severity:'error',code:'PROTECTED_PAGE_INDEXABLE',route,file:item.file});
}
...
const errors=findings.filter(x=>x.severity==='error');const warnings=findings.filter(x=>x.severity==='warning');
const report={version:'PUBLIC_PORTAL_AUDIT_V1_20260713',generatedAt:new Date().toISOString(),summary:{htmlFiles:htmlFiles.length,pages:pages.length,errors:errors.length,warnings:warnings.length},findings,pages};
fs.mkdirSync(path.dirname(REPORT),{recursive:true});fs.writeFileSync(REPORT,JSON.stringify(report,null,2));
console.log(JSON.stringify(report.summary,null,2));
if(errors.length){console.error('\nCritical audit findings:');for(const x of errors.slice(0,100))console.error(`- ${x.code} ${x.route||''} ${x.value||''} (${x.file||''})`);process.exit(1)}
