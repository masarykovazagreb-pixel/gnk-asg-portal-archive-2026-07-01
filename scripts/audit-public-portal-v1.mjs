...
const targetExists=(ref,file,baseHref='')=>{
  const clean=normalize(ref);if(external(clean)||dynamic(clean))return true;
  if(clean.startsWith('/')){if(assetFiles.has(clean))return true;const direct=path.join(ROOT,clean.slice(1));return exists(direct)||exists(path.join(direct,'index.html'))||exists(direct+'.html')}
  const base=normalize(baseHref);let baseDir=path.dirname(file);
  if(base&&!external(base))baseDir=base.startsWith('/')?path.join(ROOT,base.slice(1)):path.resolve(path.dirname(file),base);
  const direct=path.resolve(baseDir,clean);return exists(direct)||exists(path.join(direct,'index.html'))||exists(direct+'.html');
};
...
for(const file of htmlFiles){
  const html=fs.readFileSync(file,'utf8'),route=routeFor(file),protectedRoute=isProtected(route),base=baseHref(html),refs=[...extract(html,'href'),...extract(html,'src')];
  const broken=[...new Set(refs.filter(ref=>!targetExists(ref,file,base)))],legacy=LEGACY.filter(token=>html.includes(token));
  const menuCount=(html.match(new RegExp(MENU.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'g'))||[]).length;
  const robots=html.match(/<meta\b[^>]*name=["']robots["'][^>]*content=["']([^"']+)["']/i)?.[1]||'',noindex=/noindex/i.test(robots),indexable=!protectedRoute&&!noindex;
  const canon=canonical(html),lang=html.match(/<html\b[^>]*lang=["']([^"']+)["']/i)?.[1]||'',item={route,file:posix(path.relative('.',file)),protected:protectedRoute,indexable,noindex,lang,canonical:canon,baseHref:base,broken,legacy,menuCount};pages.push(item);
...
const report={version:'PUBLIC_PORTAL_AUDIT_V4_20260713_BASE_HREF',generatedAt:new Date().toISOString(),summary:{htmlFiles:htmlFiles.length,pages:pages.length,errors:errors.length,warnings:warnings.length},findings,pages};
...
if(errors.length){for(const x of errors.slice(0,100))console.error(`- ${x.code} ${x.route||''} ${x.value||''} (${x.file||''})`);process.exit(1)}
