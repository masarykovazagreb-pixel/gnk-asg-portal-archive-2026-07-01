...
ROOT=Path.cwd(); src=ROOT/'ops'/'high-throughput-capability-extension-v7.json'; out=ROOT/'artifacts'/'high-throughput-capability-extension-v7'; out.mkdir(parents=True,exist_ok=True)
errors=[]
try:data=json.loads(src.read_text())
except Exception as e:data={};errors.append(f'cannot parse capability extension v7: {e}')
...
report={'version':'GNK_ASG_HIGH_THROUGHPUT_CAPABILITY_EXTENSION_V7_VALIDATION','ok':not errors,'capabilityCount':len(caps),'errors':errors}
