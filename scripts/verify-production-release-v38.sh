...
show_relevant_headers() {
  local headers="$1"
  grep -Ei '^(HTTP/|location:|content-type:|cache-control:|cf-cache-status:|cf-ray:|x-gnk-active-entrypoint:|x-gnk-active-release:|x-gnk-deploy-revision:|x-gnk-route-owner:|x-gnk-news-source:|x-gnk-news-share:|x-gnk-news-id:|x-gnk-market-data:|x-gnk-market-source:|x-gnk-market-route:)' "$headers" >&2 || true
}
...
verify_release_marker() {
  ...
  for attempt in $(seq 1 "$attempts"); do
    ...
    if [[ "$code" = "200" ]] &&
       ...
       has_release_proof "$headers" &&
       grep -Fqi -- "$marker" "$body" "$headers"; then
      return 0
    fi
    sleep 5
  done
...
contact_status=$(curl --silent --show-error --max-redirs 0 --output "$out/contact-ready.json" --write-out '%{http_code}' "$(request_url "${base}/api/contact-submit" "${cache}-contact")" || true)
...
echo "Production release ${revision} is active and all V38 verification gates passed."
