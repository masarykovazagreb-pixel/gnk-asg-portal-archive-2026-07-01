...
| Digital Workforce frontend — statička provjera koda | **NEEDS REVIEW** (vidi D1) |
...
### D1 — `/digital-workforce/`: sadržaj stranice tvrdi da javni pristup nije odobren

Stranica `apps/portal/digital-workforce/index.html` sadrži vidljiv tekst:
> "Zaštićeni pregled · javni pristup još nije odobren"

i `<title>` uključuje "zaštićeni pregled". Istovremeno, `robots.txt` **ne blokira** ovu putanju, i stranica nije uključena ni u jedan sitemap (namjerno, moja odluka u ovom prolazu).

**Treba vlasnikovu odluku:**
- Je li tekst zastario (odobrenje već dano, treba ukloniti napomenu sa stranice)?
- Ili je pristup stvarno još neodobren (treba dodati `Disallow: /digital-workforce/` u robots.txt kao dodatnu zaštitu)?

Predložena robots.txt izmjena (NIJE primijenjena, čeka odobrenje):
```diff
+ Disallow: /digital-workforce/
```
...
Napomena: digital-workforce UTM linkovi ne bi se trebali koristiti dok se D1 ne riješi.
